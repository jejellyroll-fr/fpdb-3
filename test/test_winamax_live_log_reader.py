...
def test_explicit_round_events_advance_the_live_street_and_pot_shape() -> None:
    pool = "cg.tamgr.cg_4.t5228"
    reader = WinamaxLiveLogReader(on_table_update=MagicMock())
    reader.process_line(f"1786129601014 inf [table] 9 {pool} hand 5228-42-1786129600\n")
    reader.process_line(f'1786129601015 inf [table] 9 {pool} action raise login="Opener" amount="0.06"\n')
    reader.process_line(f'1786129601016 inf [table] 9 {pool} action call login="Caller" amount="0.06"\n')
    reader.on_table_update.reset_mock()
...
def test_regular_table_route_carries_the_unnumbered_window_label_across_hands() -> None:
    ...
    reader.process_line(f"1786129601014 inf [table] 2 {pool} hand 86426-42-1786129600\n")
    reader.process_line(
...
def test_priming_recovers_table_label_older_than_the_recent_hand_tail() -> None:
    pool = "cg.tamgr.cg_5.t86426"
    route = f"1786129601015 inf [router] Navigate: wam://table?tblrk={pool}&label=Casablanca\n"
    hand = f"1786129602014 inf [table] 2 {pool} hand 86426-43-1786129601\n"
    reader = WinamaxLiveLogReader()
    reader.PRIME_BYTES = len(hand) + 5

    reader._prime_from_tail(StringIO(route + "irrelevant event\n" * 100 + hand))

    assert reader.get_table(pool).table_label == "Casablanca"


def test_new_hand_resets_the_street_and_preflop_raiser() -> None:
    pool = "cg.tamgr.cg_4.t5228"
    reader = WinamaxLiveLogReader()
    reader.process_line(f"1786129601014 inf [table] 9 {pool} hand 5228-42-1786129600\n")
    reader.process_line(f'1786129601015 inf [table] 9 {pool} action raise login="Opener"\n')
    reader.process_line(f'1786129601017 inf [table] 9 {pool} round flop board="As,Kd,7c"\n')
    reader.process_line(f"1786129602014 inf [table] 9 {pool} hand 5228-43-1786129601\n")
