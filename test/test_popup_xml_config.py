...
class TestPopupXMLConfiguration(unittest.TestCase):
    ...
    def test_popup_windows_section_exists(self):
        """Test that popup_windows section exists."""
        popup_windows = self.root.find('popup_windows')
        self.assertIsNotNone(popup_windows, "popup_windows section not found in XML")
    
    def test_modern_popups_are_defined(self):
        ...
        for popup_name in expected_modern_popups:
            self.assertIn(popup_name, popup_names,
                f"Modern popup '{popup_name}' not found in configuration")
    
    def test_modern_popups_use_modern_classes(self):
        """Test that modern popups use ModernSubmenu classes."""
        popup_windows = self.root.find('popup_windows')
        modern_classes = ['ModernSubmenu', 'ModernSubmenuLight', 'ModernSubmenuClassic']
        
        modern_popup_names = [
            'holdring_modern', 'holdring_modern_light', 'cash_6max_complete',
            'tournament_middle', 'omaha_complete', 'stud_complete'
        ]
        ...
        for pu in popup_windows.findall('pu'):
            ...
            if pu_name in modern_popup_names:
                self.assertIn(pu_class, modern_classes,
                    f"Modern popup '{pu_name}' should use ModernSubmenu class, got '{pu_class}'")
    
    def test_popup_stat_elements(self):
        ...
        for pu in popup_windows.findall('pu'):
            ...
            self.assertGreater(len(pu_stats), 0,
                f"Popup '{pu_name}' has no pu_stat elements")
            
            # Check that each pu_stat has pu_stat_name
            for pu_stat in pu_stats:
                stat_name = pu_stat.get('pu_stat_name')
                self.assertIsNotNone(stat_name,
                    f"pu_stat in popup '{pu_name}' missing pu_stat_name attribute")
                self.assertNotEqual(stat_name.strip(), "",
                    f"pu_stat in popup '{pu_name}' has empty pu_stat_name")
    
    def test_comprehensive_popup_stats(self):
        ...
        comprehensive_popups = {
            'cash_6max_complete': {
                'required_stats': ['playername', 'vpip', 'pfr', 'cb1', 'steal'],
                'min_stats': 30
            },
            'postflop_specialist': {
                'required_stats': ['cb1', 'cb2', 'cb3', 'f_cb1', 'float_bet'],
                'min_stats': 20
            },
            'preflop_specialist': {
                'required_stats': ['vpip', 'pfr', 'three_B', 'rfi_early_position'],
                'min_stats': 15
            }
        }
        ...
        for pu in popup_windows.findall('pu'):
            ...
            if pu_name in comprehensive_popups:
                ...
                for required_stat in config['required_stats']:
                    self.assertIn(required_stat, stat_names,
                        f"Popup '{pu_name}' missing required stat '{required_stat}'")
                
                # Check minimum number of stats
                self.assertGreaterEqual(len(stat_names), config['min_stats'],
                    f"Popup '{pu_name}' should have at least {config['min_stats']} stats, got {len(stat_names)}")
    
    def test_tournament_popups_have_m_ratio(self):
        ...
        for pu in popup_windows.findall('pu'):
            ...
            if pu_name in tournament_popups:
                ...
                self.assertIn('m_ratio', stat_names,
                    f"Tournament popup '{pu_name}' should include m_ratio stat")
    
    def test_all_popups_have_player_info(self):
    ...
    def test_stat_sets_use_modern_popups(self):
        ...
        for ss in stat_sets.findall('ss'):
            ...
            if ss_name in modern_popup_usage:
                ...
                playershort_stats = [
                    stat for stat in ss.findall('stat') 
                    if stat.get('_stat_name') == 'playershort'
                ]
                
                self.assertGreater(len(playershort_stats), 0,
                    f"Stat set '{ss_name}' should have playershort stat")
                
                for stat in playershort_stats:
                    popup = stat.get('popup')
                    self.assertEqual(popup, expected_popup,
...
class TestPopupStatValidation(unittest.TestCase):
    ...
    def get_known_stats(self) -> Set[str]:
        ...
        known_stats = {
            'playername', 'player_note', 'n', 'vpip', 'pfr', 'three_B', 'four_B',
            'f_3bet', 'fold_vs_4bet', 'limp', 'cold_call', 'iso', 'rfi_total',
            'rfi_early_position', 'rfi_middle_position', 'rfi_late_position',
            'cb1', 'cb2', 'cb3', 'cb4', 'f_cb1', 'f_cb2', 'f_cb3', 'f_cb4',
            'cb_ip', 'cb_oop', 'triple_barrel', 'float_bet', 'probe_bet',
            'probe_bet_turn', 'probe_bet_river', 'check_raise_frequency',
            'bet_frequency_flop', 'bet_frequency_turn', 'raise_frequency_flop',
            'raise_frequency_turn', 'avg_bet_size_flop', 'avg_bet_size_turn',
            'avg_bet_size_river', 'overbet_frequency', 'steal', 'f_steal',
            'call_vs_steal', 'three_bet_vs_steal', 'resteal', 'wtsd', 'wmsd',
            'sd_winrate', 'non_sd_winrate', 'river_call_efficiency', 'totalprofit',
            'profit100', 'agg_fact', 'agg_freq', 'agg_pct', 'a_freq_123',
            'm_ratio', 'saw_f', 'game_abbr', 'hands', 'three_bet_range',
            'vpip_pfr_ratio', 'blank'
        }
        return known_stats
    
    def test_all_popup_stats_are_valid(self):
        ...
        if invalid_stats:
            self.fail(f"Invalid stats found in modern popups:\n" + "\n".join(invalid_stats))
    
    def test_popup_stat_categorization(self):
        ...
        for pu in popup_windows.findall('pu'):
            ...
            if pu_name in category_tests:
                ...
                for expected_cat in test_config['expected_categories']:
                    self.assertIn(expected_cat, stat_categories,
                ...
                for forbidden_cat in test_config['forbidden_categories']:
                    self.assertNotIn(forbidden_cat, stat_categories,
                        f"Popup '{pu_name}' should not contain stats from category '{forbidden_cat}'")
    
    def test_variant_specific_popups(self):
...
class TestPopupUsageInStatSets(unittest.TestCase):
    ...
    def test_all_referenced_popups_exist(self):
    ...
    def test_modern_stat_sets_use_modern_popups(self):
        ...
        modernized_stat_sets = [
            'holdem_6max_pro', 'holdem_9max_pro', 'holdem_headsup_pro',
            'holdem_tournament_pro', 'omaha_cg_expert', 'stud_cg_expert'
        ]
                ...
                for stat in ss.findall('stat'):
                    ...
                    if popup and popup in popup_class_map:
                        ...
                        if stat_name in ['playershort'] and popup_class not in modern_popup_classes:
                            self.fail(f"Stat set '{ss_name}' stat '{stat_name}' uses non-modern popup '{popup}' (class: {popup_class})")
    
    def test_popup_consistency_across_variants(self):
        ...
        omaha_sets = [ss for ss in stat_sets.findall('ss') if 'omaha' in ss.get('name', '')]
        stud_sets = [ss for ss in stat_sets.findall('ss') if 'stud' in ss.get('name', '')]
        ...
        for ss_name, stats in holdem_usage.items():
            if 'playershort' in stats:
                ...
                self.assertTrue(
                    popup.startswith('cash_') or popup.startswith('tournament_') or 
                    popup in ['holdring_main', 'holdring_modern', 'holdtour_main'],
