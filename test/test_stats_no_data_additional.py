...
class TestFoldStealNoDataFeature:
    ...
    def test_f_steal_with_no_opportunities(self):
        """Test f_steal returns '-' when no steal attempts."""
        stat_dict = {
            'player1': {'sbstolen': 0, 'bbstolen': 0, 'sbnotdef': 0, 'bbnotdef': 0}
        }
    ...
    def test_f_steal_with_zero_but_opportunities(self):
        """Test f_steal returns '0.0' when player never folded but had steal attempts."""
        stat_dict = {
            'player1': {'sbstolen': 3, 'bbstolen': 2, 'sbnotdef': 0, 'bbnotdef': 0}
        }
        result = f_steal(stat_dict, 'player1')
    ...
    def test_f_steal_with_normal_value(self):
        """Test f_steal returns normal percentage when player folded to steals."""
        stat_dict = {
            'player1': {'sbstolen': 3, 'bbstolen': 2, 'sbnotdef': 2, 'bbnotdef': 1}
        }
        result = f_steal(stat_dict, 'player1')
    ...
    def test_f_steal_exception_handling(self):
...
class TestStealSuccessNoDataFeature:
    ...
    def test_s_steal_with_no_attempts(self):
        """Test s_steal returns '-' when no steal attempts."""
        stat_dict = {
            'player1': {'steal': 0, 'suc_st': 0}
        }
    ...
    def test_s_steal_exception_handling(self):
...
class TestRaiseToStealNoDataFeature:
    ...
    def test_raise_to_steal_with_no_opportunities(self):
        """Test raiseToSteal returns '-' when no opportunities."""
        stat_dict = {
            'player1': {'rts_opp': 0, 'rts': 0}
        }
    ...
    def test_raise_to_steal_with_zero_but_opportunities(self):
        """Test raiseToSteal returns '0.0' when player never raised but had opportunities."""
        stat_dict = {
            'player1': {'rts_opp': 6, 'rts': 0}
        }
        result = raiseToSteal(stat_dict, 'player1')
    ...
    def test_raise_to_steal_exception_handling(self):
...
class TestVPIPPFRRatioNoDataFeature:
    ...
    def test_vpip_pfr_ratio_with_no_opportunities(self):
        """Test vpip_pfr_ratio returns '-' when no preflop opportunities."""
        stat_dict = {
            'player1': {'vpip_opp': 0, 'pfr_opp': 0, 'vpip': 0, 'pfr': 0}
        }
    ...
    def test_vpip_pfr_ratio_with_vpip_only_opportunities(self):
        """Test vpip_pfr_ratio returns '-' when only VPIP opportunities."""
        stat_dict = {
            'player1': {'vpip_opp': 10, 'pfr_opp': 0, 'vpip': 3, 'pfr': 0}
        }
        result = vpip_pfr_ratio(stat_dict, 'player1')
    ...
    def test_vpip_pfr_ratio_with_pfr_only_opportunities(self):
        """Test vpip_pfr_ratio returns '-' when only PFR opportunities."""
        stat_dict = {
            'player1': {'vpip_opp': 0, 'pfr_opp': 10, 'vpip': 0, 'pfr': 2}
        }
        result = vpip_pfr_ratio(stat_dict, 'player1')
    ...
    def test_vpip_pfr_ratio_with_normal_value(self):
        """Test vpip_pfr_ratio returns normal ratio when player has both stats."""
        stat_dict = {
            'player1': {'vpip_opp': 20, 'pfr_opp': 20, 'vpip': 6, 'pfr': 4}
        }
        result = vpip_pfr_ratio(stat_dict, 'player1')
    ...
    def test_vpip_pfr_ratio_with_zero_pfr(self):
        """Test vpip_pfr_ratio handles zero PFR correctly."""
        stat_dict = {
            'player1': {'vpip_opp': 20, 'pfr_opp': 20, 'vpip': 4, 'pfr': 0}
        }
        result = vpip_pfr_ratio(stat_dict, 'player1')
    ...
    def test_vpip_pfr_ratio_exception_handling(self):
...
class TestAdditionalStatsIntegration:
    ...
    def test_new_player_additional_stats_no_data(self):
        """Test all new additional stats return '-' for a completely new player."""
        stat_dict = {
            'new_player': {
                'sbstolen': 0, 'bbstolen': 0, 'sbnotdef': 0, 'bbnotdef': 0,
                'steal': 0, 'suc_st': 0,
                'rts_opp': 0, 'rts': 0,
                'vpip_opp': 0, 'pfr_opp': 0, 'vpip': 0, 'pfr': 0,
            }
        }
    ...
    def test_active_player_with_real_zeros(self):
        ...
        stat_dict = {
            'active_player': {
                'sbstolen': 5, 'bbstolen': 3, 'sbnotdef': 0, 'bbnotdef': 0,  # Always defended
                'steal': 12, 'suc_st': 0,  # Steals but never successful
                'rts_opp': 8, 'rts': 0,    # Never raised to steal
                'vpip_opp': 50, 'pfr_opp': 50, 'vpip': 15, 'pfr': 5,  # 30% VPIP, 10% PFR
            }
        }
    ...
    def test_mixed_scenario_additional_stats(self):
        """Test realistic scenario with some additional stats having data, others not."""
        stat_dict = {
            'mixed_player': {
                'sbstolen': 0, 'bbstolen': 0, 'sbnotdef': 0, 'bbnotdef': 0,  # No steal attempts
                'steal': 8, 'suc_st': 5,     # Some steal success
                'rts_opp': 6, 'rts': 2,      # Some raise to steal
                'vpip_opp': 40, 'pfr_opp': 40, 'vpip': 12, 'pfr': 8,  # 30% VPIP, 20% PFR
            }
        }
...
class TestAdditionalStatsRegressionTests:
    ...
    def test_f_steal_exception_handling_complete(self):
    ...
    def test_s_steal_exception_handling_complete(self):
    ...
    def test_raise_to_steal_exception_handling_complete(self):
    ...
    def test_vpip_pfr_ratio_exception_handling_complete(self):
