...
def hand():
    ...
    hand = HoldemOmahaHand(config_mock, None, "Winamax", gametype, "", builtFrom="Test")
    hand.players = [
        [1, "Player1", "10000", "B", None],
        [2, "Player2", "10000", "S", None],
    ]
    ...
    hand.actions = {
        ...
        "RIVER": [("Player1", "checks"), ("Player2", "checks")],
    }
...
def test_continuation_bet_flop(stats_calculator):
    ...
    hand_cbet.players = [
        ...
        [2, "Player2", "10000", "B", None],
    ]  # P1 on Bouton/CO, P2 on BB
    ...
    hand_cbet.actions = {
        "BLINDSANTES": [
            ("Player1", "small blind", Decimal("10"), False),
            ("Player2", "big blind", Decimal("20"), False),
        ],
        "PREFLOP": [
            ("Player1", "raises", Decimal("40"), Decimal("60"), Decimal("20"), False),
            ("Player2", "calls", Decimal("40"), False),
        ],
        "FLOP": [
            ("Player2", "checks"),
            ("Player1", "bets", Decimal("50"), False),
            ("Player2", "folds"),
        ],  # P1 CBet Flop
        "TURN": [],
        "RIVER": [],
...
def test_check_raise_flop(stats_calculator):
    ...
    hand_cr.actions = {
        ...
        "FLOP": [
            ...
            ("Player1", "raises", Decimal("70"), Decimal("100"), Decimal("30"), False),
        ],  # P1 Check-Raise Flop
        "TURN": [],
        "RIVER": [],
    }
...
def test_discard_counter(stats_calculator):
    ...
    hand_draw.actions = {
        "BLINDSANTES": [],
        "DEAL": [
            ("Player1", "bets", Decimal("20"), False),
            ("Player2", "calls", Decimal("20"), False),
        ],
        "DRAWONE": [
            ("Player1", "discards", 2),
            ("Player2", "stands pat"),
    ...
    assert (
        player1_stats.get("street1Discards", 0) == 2
    ), "Player1 should have discarded 2 cards on DRAWONE"
    assert (
        player1_stats.get("street2Discards", 0) == 0
    ), "Player1 should have 0 discard actions on DRAWTWO"
    assert (
        player1_stats.get("street3Discards", 0) == 0
    ), "Player1 should have 0 discard actions on DRAWTHREE"

    # Player2 discards 1 card on DRAWTHREE (street 3)
    assert (
        player2_stats.get("street1Discards", 0) == 0
    ), "Player2 should have 0 discard actions on DRAWONE"
    assert (
        player2_stats.get("street2Discards", 0) == 0
    ), "Player2 should have 0 discard actions on DRAWTWO"
    assert (
        player2_stats.get("street3Discards", 0) == 1
    ), "Player2 should have discarded 1 card on DRAWTHREE"
...
def test_aggression_flag_logic(stats_calculator):
    ...
    hand_aggr.actions = {
        ...
        "FLOP": [
            ("Player1", "bets", Decimal("30"), False),
            ("Player2", "calls", Decimal("30"), False),
            ("Player1", "checks"),
        ],  # P1 Bet then Check Flop
        "TURN": [
            ("Player2", "checks"),
            ("Player1", "raises", Decimal("60"), Decimal("90"), Decimal("30"), False),
            ("Player2", "folds"),
        ],  # P1 Raise then P2 Fold Turn
        "RIVER": [],
    }
