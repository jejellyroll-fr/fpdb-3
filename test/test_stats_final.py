...
class TestRFIByPositionStats:
    ...
    def test_rfi_early_position_with_no_opportunities(self):
        """Test rfi_early_position returns '-' when no opportunities."""
        stat_dict = {
            'player1': {'pfr_opp': 0, 'pfr': 0, 'tb_0': 0}
        }
    ...
    def test_rfi_early_position_with_normal_value(self):
    ...
    def test_rfi_middle_position_with_normal_value(self):
    ...
    def test_rfi_late_position_with_normal_value(self):
    ...
    def test_rfi_position_exception_handling(self):
...
class TestAvgBetSizeStats:
    ...
    def test_avg_bet_size_flop_with_no_bets(self):
        """Test avg_bet_size_flop returns '-' when no flop bets."""
        stat_dict = {
            'player1': {'street1Bets': 0, 'saw_f': 10}
        }
    ...
    def test_avg_bet_size_flop_with_bets(self):
        """Test avg_bet_size_flop returns estimated size when player bet flop."""
        stat_dict = {
            'player1': {'street1Bets': 8, 'saw_f': 20}
        }
        result = avg_bet_size_flop(stat_dict, 'player1')
    ...
    def test_avg_bet_size_turn_with_bets(self):
        """Test avg_bet_size_turn returns estimated size when player bet turn."""
        stat_dict = {
            'player1': {'street2Bets': 5, 'saw_t': 15}
        }
    ...
    def test_avg_bet_size_river_with_bets(self):
        """Test avg_bet_size_river returns estimated size when player bet river."""
        stat_dict = {
            'player1': {'street3Bets': 3, 'saw_r': 10}
        }
    ...
    def test_avg_bet_size_exception_handling(self):
...
class TestOverbetFrequencyStats:
    ...
    def test_overbet_frequency_with_no_bets(self):
        """Test overbet_frequency returns '-' when no bets."""
        stat_dict = {
            'player1': {'street1Bets': 0, 'street2Bets': 0, 'street3Bets': 0}
        }
    ...
    def test_overbet_frequency_with_bets(self):
        """Test overbet_frequency returns estimated frequency when player bet."""
        stat_dict = {
            'player1': {'street1Bets': 10, 'street2Bets': 8, 'street3Bets': 5}
        }
        result = overbet_frequency(stat_dict, 'player1')
    ...
    def test_overbet_frequency_exception_handling(self):
...
class TestFinalStatsIntegration:
    ...
    def test_new_player_all_stats_no_data(self):
        ...
        stat_dict = {
            'new_player': {
                'pfr_opp': 0, 'pfr': 0, 'tb_0': 0,
                'street1Bets': 0, 'street2Bets': 0, 'street3Bets': 0,
                'saw_f': 0, 'saw_t': 0, 'saw_r': 0
            }
        }
    ...
    def test_tight_player_profile(self):
        """Test final stats for a tight player profile."""
        stat_dict = {
            'tight_player': {
                # Low RFI from all positions
                'pfr_opp': 200, 'pfr': 24, 'tb_0': 4,  # RFI = 20, 10% total
                # Conservative betting
                'street1Bets': 6, 'street2Bets': 4, 'street3Bets': 2,
                'saw_f': 40, 'saw_t': 25, 'saw_r': 15
            }
        }
    ...
    def test_aggressive_player_profile(self):
        ...
        stat_dict = {
            'aggressive_player': {
                ...
                'pfr_opp': 150, 'pfr': 45, 'tb_0': 6,  # RFI = 39, 26% total
                # Aggressive betting
                'street1Bets': 20, 'street2Bets': 15, 'street3Bets': 10,
                'saw_f': 60, 'saw_t': 45, 'saw_r': 30
            }
        }
...
class TestFinalStatsRegressionTests:
    ...
    def test_all_final_stats_exception_handling(self):
    ...
    def test_tuple_format_consistency(self):
        ...
        stat_dict = {
            'test_player': {
                'pfr_opp': 100, 'pfr': 20, 'tb_0': 5,
                'street1Bets': 10, 'street2Bets': 8, 'street3Bets': 5,
                'saw_f': 40, 'saw_t': 30, 'saw_r': 20
            }
        }
