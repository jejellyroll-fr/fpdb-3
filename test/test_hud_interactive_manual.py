...
class HUDManualTestWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        """)
        instructions.setStyleSheet("font-size: 12px; margin: 10px; color: #666666; background-color: #F0F0F0; padding: 10px;")
    ...
    def create_test_scenarios(self, layout):
        ...
        scenarios = [
            ...
            },
            {
                ...
                "data": {
                    1: {
                        'vpip': 0, 'vpip_opp': 25,     # 0 VPIP from 25 opportunities
                        'pfr': 0, 'pfr_opp': 25,       # 0 PFR from 25 opportunities
                        'tb_0': 0, 'tb_opp_0': 8,      # 0 3-bets from 8 opportunities
                        'steal': 0, 'steal_opp': 10,   # 0 steals from 10 opportunities
                        'cb_1': 0, 'cb_opp_1': 0,      # No c-bet opportunities
                        'aggr_1': 0, 'saw_f': 12       # 0 aggression, saw 12 flops
                    }
                },
            ...
            },
            {
                ...
                "data": {
                    1: {
                        'vpip': 12, 'vpip_opp': 50,    # 24% VPIP
                        'pfr': 8, 'pfr_opp': 50,       # 16% PFR
                        'tb_0': 3, 'tb_opp_0': 15,     # 20% 3-bet
                        'steal': 4, 'steal_opp': 12,   # 33% steal
                        'cb_1': 6, 'cb_opp_1': 10,     # 60% c-bet
                        'aggr_1': 8, 'saw_f': 20       # 40% aggression frequency
                    }
                },
            ...
            },
            {
                ...
                "data": {
                    1: {
                        ...
                        'pfr': 0, 'pfr_opp': 15,       # Tight -> '0.0'
                        'tb_0': 2, 'tb_opp_0': 10,     # Normal -> '20.0'
                        'steal': 0, 'steal_opp': 0,    # No data -> '-'
                        'cb_1': 5, 'cb_opp_1': 8,      # Active -> '62.5'
                        'aggr_1': 0, 'saw_f': 0        # No data -> '-'
                    }
                },
    ...
    def create_scenario_widget(self, layout, scenario):
        """Create a widget for a specific test scenario."""
        # Create scenario container
        scenario_widget = QWidget()
        scenario_widget.setStyleSheet(f"background-color: {scenario['color']}; border: 1px solid #CCCCCC; margin: 5px; padding: 5px;")
        ...
        for stat_name, stat_label in zip(stat_names, stat_labels):
            ...
            stat_container.setStyleSheet("background-color: #FFFFFF; border: 1px solid #DDDDDD; margin: 2px; padding: 2px;")
...
def main():
    ...
    print("HUD Manual Test Window opened.")
    print("Please verify the display and close the window when done.")
