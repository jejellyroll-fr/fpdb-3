...
class TestStatSetSwitching(unittest.TestCase):
    ...
    def test_get_current_stat_set(self):
        ...
        self.assertEqual(current_stat_set, "Current_StatSet")

    def test_create_stat_sets_dict(self):
        ...
        self.assertEqual(stat_sets_dict, expected)

    def test_change_stat_set_successful_refresh(self):
        ...
        with patch('Aux_Hud.log') as mock_log:
            ...
            self.assertEqual(self.aux_window.game_params, new_game_params)
    ...
    def test_change_stat_set_refresh_failure_restarts_hud(self):
    ...
    def test_refresh_stats_layout_method(self):
        ...
        self.assertEqual(simple_hud.nrows, 4)
        self.assertEqual(simple_hud.ncols, 2)
        self.assertEqual(simple_hud.xpad, 5)
        self.assertEqual(simple_hud.ypad, 7)
        
        # Verify arrays recreated with correct size
        self.assertEqual(len(simple_hud.stats), 4)
        self.assertEqual(len(simple_hud.stats[0]), 2)
        self.assertEqual(len(simple_hud.popups), 4)
        self.assertEqual(len(simple_hud.popups[0]), 2)
        self.assertEqual(len(simple_hud.tips), 4)
        self.assertEqual(len(simple_hud.tips[0]), 2)
        
        # Verify stats populated correctly
        self.assertEqual(simple_hud.stats[0][0], "vpip")
        self.assertEqual(simple_hud.stats[1][1], "pfr")
        self.assertEqual(simple_hud.stats[2][0], "aggr")
        self.assertEqual(simple_hud.popups[0][0], "popup1")
        self.assertEqual(simple_hud.popups[1][1], "popup2")
        self.assertEqual(simple_hud.tips[0][0], "tip1")
        self.assertEqual(simple_hud.tips[1][1], "tip2")

    def test_update_stat_set_in_config(self):
    ...
    def test_update_xml_stat_set(self):
            ...
            if game_node.getAttribute("game_name") == "holdem":
                ...
                for gss_node in gss_nodes:
                    if gss_node.getAttribute("game_type") == "ring":
                        self.assertEqual(gss_node.getAttribute("stat_set"), "NewStatSet")

    def test_stat_set_combo_creation(self):
        ...
        with patch.object(self.popup_menu, '_get_current_stat_set', return_value="Advanced"):
            ...
            with patch.object(self.popup_menu, 'change_stat_set') as mock_change:
                combo = self.popup_menu.build_stat_set_combo(stat_sets_dict)
                
                # Verify combo box setup
                self.assertIsInstance(combo, QComboBox)
                self.assertEqual(combo.count(), 3)
                self.assertEqual(combo.itemText(0), "Basic")
                self.assertEqual(combo.itemText(1), "Advanced")
                self.assertEqual(combo.itemText(2), "Tournament")
...
class TestStatSetSwitchingIntegration(unittest.TestCase):
    ...
    def test_full_stat_set_switch_workflow(self):
    ...
    def test_stat_set_persistence_after_hand(self):
        ...
        self.assertEqual(game_params.name, "Tournament")
