...
from PopupThemes import get_theme, get_stat_color
...
class TestPopupPerformance(unittest.TestCase):
    ...
    def test_theme_creation_performance(self):
        ...
        self.assertLess(execution_time, 0.1, 
            f"Theme creation too slow: {execution_time:.3f}s for 100 themes")
        
        # Verify all themes were created
        self.assertEqual(len(themes), 100)
    
    def test_icon_provider_performance(self):
        """Test icon provider performance."""
        def get_many_icons():
            icons = []
            test_stats = [
                'vpip', 'pfr', 'three_B', 'cb1', 'f_cb1', 'steal', 'f_steal',
                'agg_fact', 'hands', 'totalprofit', 'wtsd', 'wmsd'
            ]
        ...
        self.assertLess(execution_time, 0.05,
            f"Icon retrieval too slow: {execution_time:.3f}s for 1200 icons")
        
        # Verify all icons were retrieved
        self.assertEqual(len(icons), 1200)
    
    def test_stat_categorization_performance(self):
        """Test stat categorization performance."""
        def categorize_many_stats():
            categories = []
            test_stats = [
                'playername', 'vpip', 'pfr', 'three_B', 'four_B', 'limp',
                'cb1', 'cb2', 'cb3', 'f_cb1', 'f_cb2', 'float_bet',
                'steal', 'f_steal', 'resteal', 'agg_fact', 'hands',
                'totalprofit', 'wtsd', 'wmsd', 'unknown_stat'
            ]
        ...
        self.assertLess(execution_time, 0.03,
            f"Stat categorization too slow: {execution_time:.3f}s for 2100 categorizations")
        
        # Verify all categorizations were performed
        self.assertEqual(len(categories), 2100)
    ...
    def test_color_calculation_performance(self):
        ...
        def calculate_many_colors():
            colors = []
            test_cases = [
                ('vpip', 15.0), ('vpip', 28.0), ('vpip', 45.0),
                ('pfr', 10.0), ('pfr', 20.0), ('pfr', 35.0),
                ('three_B', 2.0), ('three_B', 6.0), ('three_B', 12.0),
                ('agg_fact', 1.0), ('agg_fact', 2.5), ('agg_fact', 4.0)
            ]
        ...
        self.assertLess(execution_time, 0.02,
            f"Color calculation too slow: {execution_time:.3f}s for 1200 calculations")
        
        # Verify all colors were calculated
        self.assertEqual(len(colors), 1200)
    
    def test_large_stat_set_performance(self):
        ...
        def process_large_stat_set():
            ...
            large_stat_list = [
                'playername', 'player_note', 'n', 'saw_f', 'wtsd', 'wmsd',
                'cb1', 'f_cb1', 'float_bet', 'check_raise_frequency',
                'bet_frequency_flop', 'raise_frequency_flop', 'cb2', 'f_cb2',
                'probe_bet_turn', 'bet_frequency_turn', 'raise_frequency_turn',
                'cb3', 'f_cb3', 'probe_bet_river', 'river_call_efficiency',
                'avg_bet_size_flop', 'avg_bet_size_turn', 'avg_bet_size_river',
                'overbet_frequency', 'triple_barrel', 'cb_ip', 'cb_oop',
                'sd_winrate', 'non_sd_winrate'
            ]
        ...
        self.assertLess(execution_time, 0.005,
            f"Large stat set processing too slow: {execution_time:.3f}s for 30 stats")
    ...
    def test_theme_switching_performance(self):
        ...
        self.assertLess(execution_time, 0.05,
            f"Theme switching too slow: {execution_time:.3f}s for 150 switches")
        
        # Verify all themes were created
        self.assertEqual(len(themes), 150)
    
    def test_icon_provider_switching_performance(self):
        ...
        self.assertLess(execution_time, 0.03,
            f"Provider switching too slow: {execution_time:.3f}s for 150 switches")
        
        # Verify all providers were created
        self.assertEqual(len(providers), 150)
    ...
    def test_concurrent_operations_performance(self):
        ...
        self.assertLess(execution_time, 0.05,
            f"Concurrent operations too slow: {execution_time:.3f}s for 500 operations")
    ...
    def test_memory_efficiency(self):
        ...
        self.assertLess(execution_time, 0.1,
            f"Object creation too slow: {execution_time:.3f}s for 1000 objects")
...
class TestPopupScalability(unittest.TestCase):
    ...
    def test_stat_count_scalability(self):
        ...
        def process_n_stats(n):
            ...
            for i in range(n):
                ...
                icon = self.icon_provider.get_icon(stat_name)
                category = get_stat_category(stat_name)
                color = get_stat_color(self.theme, stat_name, i % 100)
        ...
        if len(times) >= 2:
            ...
            self.assertLess(time_ratio, count_ratio * 2,
                f"Performance scaling poor: {time_ratio:.2f}x time for {count_ratio}x stats")
    
    def test_theme_complexity_scalability(self):
        ...
        self.assertLess(execution_time, 0.02,
            f"Complex theme access too slow: {execution_time:.3f}s for 1600 accesses")
        
        # Verify all accesses completed
        self.assertEqual(result, 1600)
    
    def test_popup_count_scalability(self):
        ...
        self.assertLess(execution_time, 0.05,
            f"Multiple popup creation too slow: {execution_time:.3f}s for 10 popups")
        
        # Verify popups were created
        self.assertEqual(len(popups), 10)
        for popup in popups:
            self.assertEqual(len(popup['stats']), 20)
