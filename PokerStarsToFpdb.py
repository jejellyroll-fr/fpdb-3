...
class PokerStars(HandHistoryConverter):
    ...
    re_Identify = re.compile(
        "(PokerStars|POKERSTARS|Hive\sPoker|Full\sTilt|PokerMaster|Run\sIt\sOnce\sPoker|BetOnline|PokerBros|MPLPoker|SupremaPoker)(\sGame|\sHand|\sHome\sGame|\sHome\sGame\sHand|Game|\s(Zoom|Rush)\sHand|\sGAME)\s\#\d+:"
    )
    ...
    re_PostSB = re.compile(
        r"%(PLYR)s: posts small blind %(CUR)s(?P<SB>[,.0-9]+)" % substitutions,
        re.MULTILINE,
    )
    re_PostBB = re.compile(
        r"%(PLYR)s: posts big blind %(CUR)s(?P<BB>[,.0-9]+)" % substitutions,
        re.MULTILINE,
    )
    re_PostBUB = re.compile(
        r"%(PLYR)s: posts button blind %(CUR)s(?P<BUB>[,.0-9]+)" % substitutions,
        re.MULTILINE,
    )
    re_Antes = re.compile(
        r"%(PLYR)s: posts the ante %(CUR)s(?P<ANTE>[,.0-9]+)" % substitutions,
        re.MULTILINE,
    )
    ...
    re_PostBoth = re.compile(
        r"%(PLYR)s: posts small \& big blinds %(CUR)s(?P<SBBB>[,.0-9]+)"
        % substitutions,
        re.MULTILINE,
    )
    re_PostStraddle = re.compile(
        r"%(PLYR)s: posts straddle %(CUR)s(?P<STRADDLE>[,.0-9]+)" % substitutions,
        re.MULTILINE,
    )
    try:
        re_Action = re.compile(
            r"""%(PLYR)s:(?P<ATYPE>\sbets|\schecks|\sraises|\scalls|\sfolds|\sdiscards|\sstands\spat)(\s%(CUR)s(?P<BET>[,.\d]+))?(\sto\s%(CUR)s(?P<BETTO>[,.\d]+))?\s*(and\sis\sall.in)?(and\shas\sreached\sthe\s[%(CUR)s\d\.,]+\scap)?(\son|\scards?)?(\s\(disconnect\))?(\s\[(?P<CARDS>.+?)\])?\s*$"""
            % substitutions,
            re.MULTILINE | re.VERBOSE,
        )
    ...
    re_ShowdownAction = re.compile(
        r"%s: (shows|mucks|mucked|showed) \[(?P<CARDS>.*)\]" % substitutions["PLYR"],
        re.MULTILINE,
    )
    ...
    re_CollectPot = re.compile(
        r"Seat (?P<SEAT>[0-9]+): %(PLYR)s %(BRKTS)s(collected|showed \[.*\] and (won|collected)) \(?%(CUR)s(?P<POT>[,.\d]+)\)?(, mucked| with.*|)"
        % substitutions,
        re.MULTILINE,
    )
    ...
    re_RankOther = re.compile(
        r"%(PLYR)s finished the tournament in (?P<RANK>[0-9]+)(st|nd|rd|th) place$"
        % substitutions,
        re.MULTILINE,
    )
    re_Cancelled = re.compile("Hand\scancelled", re.MULTILINE)
    re_Uncalled = re.compile(
        "Uncalled bet \(%(CUR)s(?P<BET>[,.\d]+)\) returned to %(PLYR)s$"
        % substitutions,
        re.MULTILINE,
    )
    ...
    def loadHeroMappings(self):
        ...
        config_file = os.path.join(
            os.path.dirname(__file__), "pokerstars_skin_mapping.conf"
        )
        ...
        if not players <= self.compiledPlayers:  # x <= y means 'x is subset of y'
            ...
            if self.siteId == 26:
                self.re_HeroCards = re.compile(
                    r"Dealt\sto\s(?P<PNAME>(?![A-Z][a-z]+\s[A-Z]).+?)(?: \[(?P<OLDCARDS>.+?)\])?( \[(?P<NEWCARDS>.+?)\])"
                    % subst,
                    re.MULTILINE,
                )
                self.re_ShownCards = re.compile(
                    "Seat (?P<SEAT>[0-9]+): %(PLYR)s %(BRKTS)s(?P<SHOWED>showed|mucked) \[(?P<CARDS>.*)\]( and (lost|(won|collected) %(CUR)s(?P<POT>[,\.\d]+) with (?P<STRING>.+?))(,\sand\s(lost|won\s%(CUR)s[\.\d]+\swith\s(?P<STRING2>.*)))?)?$"
                    % subst,
                    re.MULTILINE,
                )
            else:
    ...
    def markStreets(self, hand):
        ...
        if hand.gametype["split"]:
            m = re.search(
                r"\*\*\* HOLE CARDS \*\*\*(?P<PREFLOP>.+(?=\*\*\* FIRST\sFLOP \*\*\*)|.+)"
                ...
                r"(\*\*\* SECOND RIVER \*\*\* \[\S\S \S\S \S\S \S\S] (?P<RIVER2>\[\S\S\].+))?",
                hand.handText,
                re.DOTALL,
            )
        ...
        if (
            ...
            if m2:
                hand.setCommunityCards(
                    street, [m2.group("C1"), m2.group("C2"), m2.group("C3")]
                )
            else:
    ...
    def readTourneyResults(self, hand):
        ...
        if self.re_Bounty.search(hand.handText) is None:
            ...
            for a in self.re_Progressive.finditer(hand.handText):
                ...
                koAmounts[a.group("PNAME")] += 100 * float(a.group("AMT"))
            ...
            if hand.koBounty > 0:
                for pname, amount in list(koAmounts.items()):
                    if pname == winner:
                        ...
                        hand.koCounts[pname] = (amount + hand.endBounty[pname]) / float(
                            hand.koBounty
                        )
                    else:
                        # end = 0
        ...
        if (
            ...
            if acts is not None and len([a for a in acts if a[1] != "folds"]) == 0:
                ...
                if m0 and float(m0.group("BET")) == float(hand.bb):
                    ...
                    adjustment = (
                        (float(hand.bb) - float(hand.sb)) if has_sb else float(hand.bb)
                    )
        ...
        if hand.runItTimes == 0 and hand.cashedOut is False:
            for m in self.re_CollectPot.finditer(post):
                ...
                if bovadaUncalled_v1 and float(pot) == (blindsantes + hand.pot.stp):
                    hand.addCollectPot(
                        player=m.group("PNAME"), pot=str(float(pot) - adjustment)
                    )
                elif bovadaUncalled_v2:
                    hand.addCollectPot(player=m.group("PNAME"), pot=str(float(pot) * 2))
                else:
    ...
    def readShownCards(self, hand):
        if self.siteId == 26:
            ...
            for found in m:
                ...
                hand.addShownCards(
                    cards=cards, player=found.group("PNAME"), shown=True, mucked=False
                )
    ...
    def getTableTitleRe(type, table_name=None, tournament=None, table_number=None):
        ...
        if type == "tour":
            ...
            log.info(
                f"Stars table_name='{table_name}', tournament='{tournament}', table_number='{table_number}'"
            )
