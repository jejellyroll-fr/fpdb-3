...
class Bovada(HandHistoryConverter):
    ...
    # Showdown summary line, e.g. "Small Blind [ME] : Mucks [Th Tc] (Two pair)"
    re_shown_hand: ClassVar[re.Pattern[str]] = re.compile(
        r"^{PLYR} (\s?\[ME\]\s)?: (?P<SHOWED>Mucks|Shows) \[(?P<CARDS>[^\]]+)\] \((?P<STRING>[^)]+)\)".format(
            **substitutions,
        ),
        re.MULTILINE,
    )
    ...
    def readShownCards(self, hand: "Hand") -> None:
        ...
        """
        text = hand.handText.replace(" [ME]", "") if hand.version == "MVS" else hand.handText
        for m in self.re_shown_hand.finditer(text):
            cards = m.group("CARDS").split(" ")
            shown = m.group("SHOWED") == "Shows"
            mucked = m.group("SHOWED") == "Mucks"
            combo = m.group("STRING")
            player = self.playerSeatFromPosition(
                "BovadaToFpdb.readShownCards",
                hand.handid,
                m.group("PNAME"),
            )
            hand.addShownCards(
