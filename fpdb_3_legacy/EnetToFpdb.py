...
class Enet(HandHistoryConverter):
    ...
    re_Identify = re.compile("^Game\\s\\#\\d+:\\s[OH]")
    re_SplitHands = re.compile("\n\n+")
    re_TailSplitHands = re.compile("(\n\n+)")
    re_Button = re.compile("Seat #(?P<BUTTON>\\d+) is the button", re.MULTILINE)
    re_Board = re.compile(r"\[?(?P<CARDS>.+)\]")
    re_DateTime = re.compile(
        """(?P<H>[0-9]+):(?P<MIN>[0-9]+):(?P<S>[0-9]+)\\s(?P<D>[0-9]{1,2})\\.(?P<M>[0-9]{1,2})\\.(?P<Y>[0-9]{4})""",
        re.MULTILINE,
    )
        ...
        if info["limitType"] == "fl" and info["bb"] is not None:
            if info["type"] == "ring":
                try:
                    ...
                    tmp = handText[0:200]
                    log.error(
                        _("EnetToFpdb.determineGameType: Lim_Blinds has no lookup for '%s' - '%s'") % (mg["BB"], tmp)
                    )
                    raise FpdbParseError
            else:
