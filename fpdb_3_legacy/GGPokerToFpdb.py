...
class GGPoker(HandHistoryConverter):
    ...
    games: ClassVar[dict[str, tuple[str, str]]] = {  # base, category
        ...
        "OM": ("hold", "omahahi"),  # Pot Limit Omaha
    }
    ...
    re_game_info = re.compile(
        """
          ...
          (?P<GAME>Holdem|Hold\'em|ShortDeck|Omaha|PLO|Omaha\\sHi/Lo|PLO\\-(5|6)|OM)\\s
    ...
    re_button = re.compile(r"Seat\s*#?\s*(?P<BUTTON>\d+)\s*is\s*the\s*button", re.MULTILINE | re.IGNORECASE)
    ...
    def markStreets(self, hand: "Hand") -> None:
        ...
        if hand.gametype["base"] in ("hold"):
            ...
            log.debug(f"markStreets: checking for HOLE CARDS: {'*** HOLE CARDS ***' in hand.handText}")
            log.debug(f"markStreets: checking for FLOP: {'*** FLOP ***' in hand.handText}")
            m = re.search(
            ...
            # Store the matched streets in hand.streets
            if m:
            ...
            # Only use re_board3 as fallback if main regex didn't capture streets
            if not hand.streets.get("FLOP") and (m1 := self.re_board3.search(hand.handText)):
                hand.streets.update(
                    {
    ...
    def _add_hole_cards_by_game_type(
        ...
        elif (street == "PREFLOP" and hand.gametype["base"] == "hold") or street == "DEAL":
    ...
    def readAction(self, hand: "Hand", street: str) -> None:
        ...
        log.debug(f"readAction: processing street {street}, using key {s}")
        log.debug(f"readAction: hand.streets[{s}] = '{hand.streets.get(s, 'NOT_FOUND')}'")
        m = self.re_action.finditer(hand.streets[s])
        action_count = 0
        for action in m:
            action_count += 1
            log.debug(f"readAction: found action {action_count}: {action.groupdict()}")
        log.debug(f"readAction: total actions found for {street}: {action_count}")
        log.debug(f"readAction: hand.actions[{street}] before processing: {len(hand.actions.get(street, []))} actions")
        ...
        for action in m:
            ...
            log.debug(f"readAction: processing action {processed_count}: {player_name} {action_type} {amount}")

            if action_type == "raises":
                # Only some sites provide the target amount for raises
                log.debug(f"readAction: calling hand.addRaiseBy({street}, {player_name}, {amount})")
                hand.addRaiseBy(street, player_name, amount)
            elif action_type == "calls":
                log.debug(f"readAction: calling hand.addCall({street}, {player_name}, {amount})")
                hand.addCall(street, player_name, amount)
            elif action_type == "bets":
                log.debug(f"readAction: calling hand.addBet({street}, {player_name}, {amount})")
                hand.addBet(street, player_name, amount)
            elif action_type == "folds":
                log.debug(f"readAction: calling hand.addFold({street}, {player_name})")
                hand.addFold(street, player_name)
            elif action_type == "checks":
                log.debug(f"readAction: calling hand.addCheck({street}, {player_name})")
    ...
    def readCollectPot(self, hand: "Hand") -> None:
        ...
        # Capture per-player cashout amounts (and the cashout risk/fee) so the
        # hand viewer and replayer can show them.
        for m in self.re_collect_pot3.finditer(pre):
            hand.cashOutAmounts[m.group("PNAME")] = Decimal(self.clearMoneyString(m.group("POT")))
        for m in self.re_collect_pot4.finditer(pre):
            hand.cashOutFees[m.group("PNAME")] = Decimal(self.clearMoneyString(m.group("POT")))
