...
def _make_session(number: int, hands: list[HandResult], graph_open_bb: float) -> SessionMetrics:
    profit_minor = sum(hand.profit_minor for hand in hands)
    ev_minor = sum(hand.all_in_ev_minor for hand in hands)
    profit_bb = sum(hand.profit_minor / hand.big_blind_minor for hand in hands if hand.big_blind_minor > 0)
    ev_bb = sum(hand.all_in_ev_minor / hand.big_blind_minor for hand in hands if hand.big_blind_minor > 0)
    ...
    cumulative_bb = graph_open_bb
    graph_high_bb = graph_low_bb = graph_open_bb
    for hand in hands:
        ...
        if hand.big_blind_minor > 0:
            cumulative_bb += hand.profit_minor / hand.big_blind_minor
        graph_high_bb = max(graph_high_bb, cumulative_bb)
        graph_low_bb = min(graph_low_bb, cumulative_bb)
    ...
    return SessionMetrics(
        ...
        profit_bb=profit_bb,
        bb_per_100=profit_bb / len(hands) * 100 if hands else 0.0,
        all_in_ev_minor=ev_minor,
        all_in_ev_bb=ev_bb,
        ev_bb_per_100=ev_bb / len(hands) * 100 if hands else 0.0,
        ev_difference_minor=ev_minor - profit_minor,
        peak_minor=peak_minor,
        low_minor=low_minor,
        max_drawdown_minor=max_drawdown_minor,
        bb_per_hour=profit_bb / hours if hours else 0.0,
        currency_per_hour=(profit_minor / 100) / hours if hours else 0.0,
        graph_open_bb=graph_open_bb,
        graph_close_bb=cumulative_bb,
        graph_high_bb=graph_high_bb,
        graph_low_bb=graph_low_bb,
    )
...
def build_sessions(rows: list[tuple[Any, ...]]) -> list[SessionMetrics]:
    ...
    for number, group in enumerate(groups, start=1):
        metrics = _make_session(number, group, graph_open_bb)
        sessions.append(metrics)
        graph_open_bb = metrics.graph_close_bb
    return sessions
...
def summarize_sessions(sessions: list[SessionMetrics]) -> dict[str, Any]:
    ...
    if not sessions:
        return {"sessions": 0, "hands": 0, "duration_seconds": 0, "profit_bb": 0.0, "bb_per_100": 0.0}
    hands = sum(session.hands for session in sessions)
    duration = sum(session.duration_seconds for session in sessions)
    profit_bb = sum(session.profit_bb for session in sessions)
    ...
    summary: dict[str, Any] = {
        ...
        "duration_seconds": duration,
        "profit_bb": profit_bb,
        "bb_per_100": profit_bb / hands * 100 if hands else 0.0,
        "bb_per_hour": profit_bb / (duration / 3600) if duration else 0.0,
