...
class HandHistoryConverter(ABC):
    ...
    _re_ftp_archive = re.compile(r"\*{20}\s#\s\d+\s\*{20,25}\s+", re.MULTILINE)
...
in_path   (default '-' = sys.stdin)
        ...
        if sitename is None:
            self.sitename = getattr(self, "sitename", "PokerStars")
        else:
        ...
        if self.site_id is None and self.sitename:
            try:
                self.site_id = self.config.get_site_id(self.sitename)
...
HandHistoryConverter: '{sitename}'
    ...
    def allHandsAsList(self):
        ...
        # Strip leading whitespace plus a stray BOM (﻿) or NUL (\x00) byte:
        # some exports (PartyPoker, Microgaming) prefix the file with one, which
        # str.lstrip() leaves in place and which then corrupts identification of
        # the first hand chunk ("could not identify as a <site> hand").
        self.obs = self.obs.lstrip("﻿\x00 \t\r\n\f\v")
        lenobs = len(self.obs)
        ...
        if self.starsArchive is True:
            self.obs = self._re_stars_archive.sub("", self.obs)

        if self.ftpArchive is True:
            # Remove  ******************** # 1 *************************
            self.obs = self._re_ftp_archive.sub("", self.obs)
        ...
        # re.split yields an empty leading element. It carries no hand and would
        # otherwise be flagged as a spurious "could not identify" partial, so drop it.
        while handlist and handlist[0].strip("﻿\x00 \t\r\n\f\v") == "":
            handlist.pop(0)
        ...
        if handlist and len(handlist[-1]) <= 50:
    ...
    def processHand(self, handText):
        ...
        if gametype is None:
            ...
            if gametype["category"] in self.import_parameters["importFilters"]:
                msg = "Skipped {} hand".format(gametype["type"])
                raise FpdbHandSkipped(msg)
            ...
            gametype.setdefault("currency", "USD")
        ...
        msg = f"Unsupported game type: {gametype}"
        raise FpdbParseError(msg)

    def isPartial(self, handText) -> bool:
        count = 0
        identify_re = getattr(self, "re_identify", getattr(self, "re_Identify", None))
        if identify_re is None:
            msg = f"{self.sitename} parser has no hand-identification regex"
            raise FpdbParseError(msg)
        for _m in identify_re.finditer(handText):
            count += 1
        return count != 1
    ...
    *** NOTE: You may find this is a more appropriate place to set hand.maxseats ***
    ...
    def getRake(self, hand) -> None:
        ...
        if hand.rake < 0 and (not hand.roundPenny or hand.rake < round) and not hand.cashedOut:
            ...
            hand.totalpot > 0
            and hand.totalpot / Decimal("4") < hand.rake
    ...
    def _warn_if_hand_missing_expected_data(self, hand) -> None:
        """Log non-fatal completeness warnings for hands that parsed cleanly."""
        try:
            issues = []
            hand_id = getattr(hand, "handid", "unknown")
            gametype = getattr(hand, "gametype", {}) or {}
            base = gametype.get("base")
            sb = Decimal(str(gametype.get("sb", "0") or "0"))
            ...
            blind_actions = actions.get("BLINDSANTES", [])
            if base in ("hold", "draw") and (sb > 0 or bb > 0) and not blind_actions:
                issues.append("no blind/ante actions")

            non_blind_action_count = sum(
                len(actions.get(street, []))
            ...
            for marker, street in (
                ("*** Flop ***", "FLOP"),
                ("*** Turn ***", "TURN"),
                ("*** River ***", "RIVER"),
            ):
                if marker in hand_text and street in board and not board.get(street):
                    issues.append(f"{street.lower()} marker present but board cards missing")

            if not issues:
                return

            msg = (
                f"[WARNING] Parsed {self.sitename} hand {hand_id} without parser errors, "
                f"but expected data is missing: {', '.join(issues)}"
            )
            self.parsing_issues.append(msg)
            log.warning(msg)
        except Exception as exc:
            log.warning("Unable to validate parsed hand completeness: %s", exc)
    ...
    def setFileType(self, filetype="text", codepage="utf8") -> None:
        self.filetype = filetype
        self.codepage = codepage

    # Import from string
    def setObs(self, text) -> None:
        self.obs = text
    ...
    def readFile(self) -> bool | None:
        ...
        if self.filetype == "text":
            ...
            # into garbage — null-separated characters — before utf-16 is reached,
            # and the hand fails to be recognised. errors="ignore" tolerates a stray
            # trailing byte (some exports drop the final NUL, giving an odd length).
            if raw_data[:2] in (b"\xff\xfe", b"\xfe\xff"):
                text = raw_data.decode("utf-16", errors="ignore").replace("\r\n", "\n")
                # Some of these utf-16 exports are double-spaced (a blank line after
                # every line, so hands are separated by four newlines). That breaks
                # blank-line hand splitters, so collapse the doubled newlines back to
                # single spacing when detected.
                if "\n\n\n\n" in text:
                    text = text.replace("\n\n", "\n")
                self.whole_file = text
                ...
                self.kodec = "utf-16"
                log.debug("File decoded as utf-16 (BOM detected)")
                return True
        ...
        if self.filetype == "xml":
            if hasattr(self, "in_path"):  # Ensure that the file path is available
                try:
                    ...
                    return True
                except HHC_XML_PARSE_ERRORS as e:
                    log.exception(f"Error while parsing XML file: {e}")
                    return False
            else:
    ...
    def guessMaxSeats(self, hand):
        ...
        if mo == 10:
            hand.gametype["maxSeats"] = 10
            return 10  # that was easy

        if hand.gametype["base"] == "stud" and mo <= 8:
            hand.gametype["maxSeats"] = 8
            return 8

        if hand.gametype["base"] == "draw" and mo <= 6:
            hand.gametype["maxSeats"] = 6
            return 6
    ...
    def getStatus(self):
        return self.numErrors == 0 and self.numPartial == 0
    ...
    def changeTimezone(time, givenTimezone, wantedTimezone):
        ...
        # This routine expects a naive datetime; pytz.localize() raises on an
        # already-aware one. Some callers (e.g. WinningSummary) tag the time
        # UTC before calling, so drop the tzinfo and reinterpret the wall-clock
        # in givenTimezone, which is what the conversion below intends.
        ...
        return localisedTime.astimezone(wantedTimezone) + datetime.timedelta(
            seconds=-3600 * ((offset) // (100)) - 60 * (offset % 100),
        )
    ...
    @staticmethod
    def getTableNoRe(tournament) -> str:
        ...
        return rf"{tournament}.+(?:Table|Torneo) (\d+)"
...
def getSiteHhc(config, sitename):
    ...
    from fpdb_3_legacy.IdentifySite import import_fpdb_module

    hhcModule = import_fpdb_module(hhcName)
...
def get_out_fh(out_path, parameters):
    if out_path == "-":
        return sys.stdout
    if parameters.get("saveStarsHH", False):
        out_dir = os.path.dirname(out_path)
        if not os.path.isdir(out_dir) and out_dir != "":
            try:
                os.makedirs(out_dir)
            except OSError as e:
                log.exception(f"Unable to create output directory {out_dir} for HHC: {e}")
            else:
                log.info(f"Created directory '{out_dir}'")
        try:
            return codecs.open(out_path, "w", "utf8")
        except OSError as e:
            log.exception(f"Output path {out_path} couldn't be opened: {e}")
            return None
    else:
        return sys.stdout
