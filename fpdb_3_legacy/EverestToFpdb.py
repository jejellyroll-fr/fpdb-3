...
class Everest(HandHistoryConverter):
    ...
    re_PlayerInfo = re.compile(
        r'<SEAT position="(?P<SEAT>[0-9]+)" name="(?P<PNAME>.+)" balance="(?P<CASH>[.0-9]+)"/>', re.MULTILINE
    )
    ...
    re_PostXB = re.compile(
        r'<BLIND position="(?P<PSEAT>[0-9]+)" amount="(?P<XB>[0-9]+)" penalty="(?P<PENALTY>[0-9]+)"\/>', re.MULTILINE
    )
    ...
    re_Action = re.compile(
        r'<(?P<ATYPE>FOLD|BET) position="(?P<PSEAT>[0-9])"( amount="(?P<BET>[.0-9]+)")?( allin="1")?/>', re.MULTILINE
    )
    re_ShowdownAction = re.compile(
        r'<cards type="SHOWN" cards="(?P<CARDS>..,..)" player="(?P<PSEAT>[0-9])"/>', re.MULTILINE
    )
    ...
    re_ShownCards = re.compile(
        r'<(?P<SHOW>SHOW|MUCK) position="(?P<PSEAT>[0-9])">(?P<CARDS>.+)?</(SHOW|MUCK)>', re.MULTILINE
    )
    ...
    def readPlayerStacks(self, hand):
        ...
        for a in m:
            ...
            stackstr = f"{stack / Decimal('100'):.2f}"
    ...
    def readAntes(self, hand):
        ...
        for player in m:
            amount = f"{Decimal(player.group('ANTE')) / Decimal('100'):.2f}"
    ...
    def readBlinds(self, hand):
        ...
        for a in self.re_PostXB.finditer(hand.handText):
            xb = Decimal(a.group("XB")) / Decimal("100")
            penalty = Decimal(a.group("PENALTY")) / Decimal("100")
            amount = f"{xb:.2f}"
            both = f"{penalty + xb:.2f}"
    ...
    def readAction(self, hand, street):
        ...
        for action in m:
            ...
            if action.group("ATYPE") == "BET":
                ...
                amountstr = f"{amount / Decimal('100'):.2f}"
                ...
                log.debug(
                    _("Unimplemented %s: '%s' '%s'") % ("readAction", action.group("PSEAT"), action.group("ATYPE"))
                )
    ...
    def readTourneyResults(self, hand):
        """Read tourney results - not implemented for Everest."""
        pass

    def readOther(self, hand):
        """Read other info - not implemented for Everest."""
        pass
