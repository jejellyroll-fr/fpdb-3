...
def metadata_queries(db_server: str) -> dict[str, str]:
    ...
    if db_server == "mysql":
        queries.update({"list_tables": "SHOW TABLES", "list_indexes": "SHOW INDEXES", "set tx level": "SET SESSION TRANSACTION\n            ISOLATION LEVEL READ COMMITTED"})
    elif db_server == "postgresql":
        queries.update({"list_tables": "SELECT table_name FROM information_schema.tables WHERE table_schema = 'public'", "list_indexes": "SELECT tablename, indexname FROM PG_INDEXES", "set tx level": "SET SESSION TRANSACTION\n            ISOLATION LEVEL READ COMMITTED"})
    elif db_server == "sqlite":
        queries.update({"list_tables": "SELECT name FROM sqlite_master\n            WHERE type='table'\n            ORDER BY name;", "list_indexes": "SELECT name FROM sqlite_master\n                                            WHERE type='index'\n                                            ORDER BY name;", "set tx level": " "})
    return queries
