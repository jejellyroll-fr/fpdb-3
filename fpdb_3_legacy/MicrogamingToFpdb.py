...
class Microgaming(HandHistoryConverter):
    ...
    re_PlayerInfo = re.compile(
        '<Seat num="(?P<SEAT>[0-9]+)" alias="(?P<PNAME>.+)" unicodealias=".+" balance="(?P<CASH>[.0-9]+)" endbalance="[.0-9]+"(?P<BUTTON>\\sdealer="true")?',
        re.MULTILINE,
    )
    ...
    re_HeroCards = re.compile(
        r'<Action seq="\d+" type="DealCards" seat="(?P<SEAT>\d+)">\s+?(?P<CARDS>(<Card value="[0-9TJQKA]+" suit="[csdh]" id="(?P<CARD>\d+)"\s?/>\s+)+)',
        re.MULTILINE,
    )
    re_Action = re.compile(
        r'<Action seq="\d+" type="(?P<ATYPE>[a-zA-Z]+)" seat="(?P<SEAT>\d+)"( value="(?P<BET>[.0-9]+)")?\s?/>',
        re.MULTILINE,
    )
    re_CollectPot = re.compile(
        r'<Seat num="(?P<SEAT>\d+)" amount="(?P<POT>[.\d]+)" pot=".+" type=".*" lowhandwin="\d+"\s?/>', re.MULTILINE
    )
    re_ShownCards = re.compile(
        r'<Action seq="\d+" type="(?P<SHOWED>ShowCards|MuckCards)" seat="(?P<SEAT>\d+)">\s+?(?P<CARDS>(<Card value="[0-9TJQKA]+" suit="[csdh]" id="(?P<CARD>\d+)"\s?/>\s+)+)',
        re.MULTILINE,
    )
    ...
    def markStreets(self, hand):
        if hand.gametype["base"] in ("hold"):
            m = re.search(
                '</Seats>(?P<PREFLOP>.+(?=<Action seq="\\d+" type="DealFlop")|.+)'
                ...
                '((?P<RIVER><Action seq="\\d+" type="DealRiver">.+?(?=<Action seq="\\d+" type="ShowCards|MuckCards")|.+))?',
                hand.handText,
                re.DOTALL,
            )
        if hand.gametype["category"] in ("27_1draw", "fivedraw"):
            m = re.search(
                r'(?P<PREDEAL>.+?(?=<ACTION TYPE="HAND_DEAL")|.+)'
                r'(<ACTION TYPE="HAND_DEAL"(?P<DEAL>.+(?=<ACTION TYPE="HAND_BOARD")|.+))?'
                r'(<ACTION TYPE="(?P<DRAWONE>.+))?',
                hand.handText,
                re.DOTALL,
            )
        # import pprint
    ...
    def readShownCards(self, hand):
        for shows in self.re_ShownCards.finditer(hand.handText):
            ...
            if shows.group("SHOWED") == "ShowCards":
                shown = True
            elif shows.group("SHOWED") == "MuckCards":
                mucked = True
            hand.addShownCards(cards, self.playerNameFromSeatNo(shows.group("SEAT"), hand), shown=shown, mucked=mucked)

    def readSTP(self, hand):
        """Read Splash the Pot - not implemented for Microgaming."""
        pass

    def readTourneyResults(self, hand):
        """Read tourney results - not implemented for Microgaming."""
        pass

    def readOther(self, hand):
        """Read other info - not implemented for Microgaming."""
        pass
