...
def hud_current_stats_queries() -> dict[str, str]:
    ...
    query["get_stats_from_hand"] = """
            SELECT hc.playerId                      AS player_id,
                ...
                /* PT4 action-enum counters (action_enum_stats.py). */
                sum(hc.cnt_p_face_3bet)             AS cnt_p_face_3bet,
                sum(hc.cnt_p_call_3bet)             AS cnt_p_call_3bet,
                sum(hc.cnt_p_raise_3bet)            AS cnt_p_raise_3bet,
                sum(hc.cnt_p_face_4bet)             AS cnt_p_face_4bet,
                sum(hc.cnt_p_call_4bet)             AS cnt_p_call_4bet,
                sum(hc.cnt_p_raise_4bet)            AS cnt_p_raise_4bet,
                sum(hc.cnt_p_face_squeeze)          AS cnt_p_face_squeeze,
                sum(hc.cnt_p_call_squeeze)          AS cnt_p_call_squeeze,
                sum(hc.cnt_p_raise_squeeze)         AS cnt_p_raise_squeeze,
                sum(hc.cnt_f_face_cbet)             AS cnt_f_face_cbet,
                sum(hc.cnt_f_call_cbet)             AS cnt_f_call_cbet,
                sum(hc.cnt_f_raise_cbet)            AS cnt_f_raise_cbet,
                sum(hc.cnt_f_face_donk)             AS cnt_f_face_donk,
                sum(hc.cnt_f_call_donk)             AS cnt_f_call_donk,
                sum(hc.cnt_f_raise_donk)            AS cnt_f_raise_donk,
                sum(hc.cnt_f_face_3bet)             AS cnt_f_face_3bet,
                sum(hc.cnt_f_call_3bet)             AS cnt_f_call_3bet,
                sum(hc.cnt_f_raise_3bet)            AS cnt_f_raise_3bet,
                sum(hc.cnt_f_face_4bet)             AS cnt_f_face_4bet,
                sum(hc.cnt_f_call_4bet)             AS cnt_f_call_4bet,
                sum(hc.cnt_f_raise_4bet)            AS cnt_f_raise_4bet,
                sum(hc.cnt_t_face_cbet)             AS cnt_t_face_cbet,
                sum(hc.cnt_t_call_cbet)             AS cnt_t_call_cbet,
                sum(hc.cnt_t_raise_cbet)            AS cnt_t_raise_cbet,
                sum(hc.cnt_t_face_donk)             AS cnt_t_face_donk,
                sum(hc.cnt_t_call_donk)             AS cnt_t_call_donk,
                sum(hc.cnt_t_raise_donk)            AS cnt_t_raise_donk,
                sum(hc.cnt_t_face_3bet)             AS cnt_t_face_3bet,
                sum(hc.cnt_t_call_3bet)             AS cnt_t_call_3bet,
                sum(hc.cnt_t_raise_3bet)            AS cnt_t_raise_3bet,
                sum(hc.cnt_t_face_4bet)             AS cnt_t_face_4bet,
                sum(hc.cnt_t_call_4bet)             AS cnt_t_call_4bet,
                sum(hc.cnt_t_raise_4bet)            AS cnt_t_raise_4bet,
                sum(hc.cnt_t_face_float)            AS cnt_t_face_float,
                sum(hc.cnt_t_call_float)            AS cnt_t_call_float,
                sum(hc.cnt_t_raise_float)           AS cnt_t_raise_float,
                sum(hc.cnt_r_face_cbet)             AS cnt_r_face_cbet,
                sum(hc.cnt_r_call_cbet)             AS cnt_r_call_cbet,
                sum(hc.cnt_r_raise_cbet)            AS cnt_r_raise_cbet,
                sum(hc.cnt_r_face_donk)             AS cnt_r_face_donk,
                sum(hc.cnt_r_call_donk)             AS cnt_r_call_donk,
                sum(hc.cnt_r_raise_donk)            AS cnt_r_raise_donk,
                sum(hc.cnt_r_face_3bet)             AS cnt_r_face_3bet,
                sum(hc.cnt_r_call_3bet)             AS cnt_r_call_3bet,
                sum(hc.cnt_r_raise_3bet)            AS cnt_r_raise_3bet,
                sum(hc.cnt_r_face_4bet)             AS cnt_r_face_4bet,
                sum(hc.cnt_r_call_4bet)             AS cnt_r_call_4bet,
                sum(hc.cnt_r_raise_4bet)            AS cnt_r_raise_4bet,
                sum(hc.cnt_r_face_float)            AS cnt_r_face_float,
                sum(hc.cnt_r_call_float)            AS cnt_r_call_float,
                sum(hc.cnt_r_raise_float)           AS cnt_r_raise_float,
