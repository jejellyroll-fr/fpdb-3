...
class PacificPokerSummary(TourneySummary):
    ...
    def getSplitRe(self, _head: str) -> re.Pattern[str]:
        """Return the regex separating tournament summaries within a file.

        Without an override TourneySummary.getSplitRe returns None, and the
        importer then calls re.split(None, ...) -> "first argument must be
        string or compiled pattern". Split on the "***** ... Tournament
        Summary *****" banner (non-capturing so re.split keeps no group text);
        re_tourney_info keys off "Tournament ID:" so dropping the banner is fine.
        """
        return re.compile(
            r"\*{5}\s(?:Cassava|888poker|888)(?:\.[a-z]{2})?(?:\-[a-z]{2})? Tournament Summary\s\*{5}",
        )
