...
from fpdb_3_legacy.HandHistoryConverter import getTableNoRe, getTableTitleRe
...
class Table_Window:
    ...
    def check_table(self):
        # Fetch geometry a single time per poll. check_size()/check_loc() each
        # call get_geometry() on their own, which on macOS can trigger an
        # expensive AppleScript window scan; doing it once halves the per-tick
        # cost and keeps the GUI thread responsive.
