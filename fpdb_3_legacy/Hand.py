...
class Hand:
    ...
    def to_canonical_dict(self):
        ...
        def safe_decimal(val):
            ...
            except HAND_DECIMAL_ERRORS:
                return Decimal("0.00")

        # We need a format kind
        kind = "cash"
        if self.tourNo or self.isSng or (self.gametype and self.gametype.get("type") == "tour"):
            if self.isSng:
                kind = "sng"
            else:
                kind = "mtt"
        ...
        game_dict = {
            "category": self.gametype.get("category", "holdem") if self.gametype else "holdem",
            "limit_type": self.gametype.get("limitType", "no_limit") if self.gametype else "no_limit",
            "currency": self.gametype.get("currency", "USD") if self.gametype else "USD",
            "small_blind": safe_decimal(self.sb),
            "big_blind": safe_decimal(self.bb)
        }

        # Format dictionary
        format_dict = {
            "kind": kind,
            "max_seats": self.maxseats,
            "table": self.tablename
        }
        if kind != "cash":
            format_dict.update({
                "tournament_id": self.tourNo,
                "buy_in": safe_decimal(self.buyin),
                "fee": safe_decimal(self.fee),
                "bounty": safe_decimal(self.koBounty)
            })

        # Datetime UTC. Parser startTime values are expected to be UTC after
        # parser-level conversion; attach an explicit UTC offset when a legacy
        # parser still provides a naive datetime.
        dt_str = canonical_datetime_utc(self.startTime)

        # Players list
        players_list = []
        for p in self.players:
            # p: [seat, name, chips, position, bounty]
            seat = p[0]
            name = p[1]
            chips = p[2]
            players_list.append({
                "seat": seat,
                "name": name,
                "starting_stack": safe_decimal(chips)
            })

        # Sort players list by seat
        players_list.sort(key=lambda x: x["seat"])
        ...
        if "BLINDSANTES" in self.actions:
            for act in self.actions["BLINDSANTES"]:
                ...
                elif action_type in ("small blind", "big blind", "both", "secondsb", "button blind", "straddle"):
                    blinds_list.append({
                        "player": player,
                        "type": action_type,
                        "amount": amount
        ...
        def clean_cards(val):
            ...
            for item in val:
                if isinstance(item, str):
                    cleaned.extend([c.strip() for c in item.split() if c.strip()])
                elif isinstance(item, (list, tuple, set)):
                    cleaned.extend(clean_cards(item))
            return cleaned
        ...
        for p in self.players:
            ...
            hc_list = []
            for street in self.holeStreets:
                if street in self.holecards and pname in self.holecards[street]:
                    opened, closed = self.holecards[street][pname]
                    for card in clean_cards(closed):
                        if card not in hc_list:
                            hc_list.append(card)
                    for card in clean_cards(opened):
                        if card not in hc_list:
                            hc_list.append(card)
            if hc_list:
                hole_cards_dict[pname] = hc_list

        # Streets and Actions
        streets_active = []
        # Filter streets to only action streets that have actions or board
        for street in self.actionStreets:
            has_act = street in self.actions and len(self.actions[street]) > 0
            has_board = street in self.board and len(clean_cards(self.board[street])) > 0
            if has_act or has_board:
                streets_active.append(street.lower())

        # Collect total action count across all action streets
        total_actions = sum(len(self.actions[s]) for s in self.actionStreets if s in self.actions)
        if "BLINDSANTES" in self.actions:
            total_actions += len(self.actions["BLINDSANTES"])

        # Board cards
        board_cards = []
        for street in self.actionStreets:
            if street in self.board:
                board_cards.extend(clean_cards(self.board[street]))

        # Showdown
        shown_list = []
        for p in self.players:
            pname = p[1]
            if pname in self.shown or pname in self.mucked:
                p_hc = hole_cards_dict.get(pname, [])
                shown_list.append({
                    "player": pname,
                    "cards": p_hc
                })
        ...
        # layers correspond to REAL side pots (distinct participant sets).
        # Hand.totalPot() subdivides fractional-value layers into $1 chunks
        # internally; those chunks share the same participant set and must
        # not be reported as side pots (they would double-count, since
        # `main` is `self.totalpot`, i.e. the sum of all layers).
        main_pot = safe_decimal(self.totalpot)
        side_pots = []
        if hasattr(self.pot, "pots") and self.pot.pots:
            # First layer becomes "main"; subsequent layers are side pots
            # only if their participant set differs from layer 0's.
            first = self.pot.pots[0]
            first_participants = first[1] if isinstance(first, (tuple, list)) and len(first) > 1 else None
            distinct_layers = []
            for p_idx, p_val in enumerate(self.pot.pots):
                if p_idx == 0:
                    continue
                val = p_val[0] if isinstance(p_val, (tuple, list)) else p_val
                parts = p_val[1] if isinstance(p_val, (tuple, list)) and len(p_val) > 1 else None
                if first_participants is not None and parts == first_participants:
                    # Same participants as layer 0: fractional chunking artifact.
                    continue
                distinct_layers.append(safe_decimal(val))
            if distinct_layers:
                # Real multi-pot hand: main is layer 0, side is the rest.
                first_val = first[0] if isinstance(first, (tuple, list)) else first
                main_pot = safe_decimal(first_val)
                side_pots = distinct_layers
                # Only add common money if we need it to balance the pot equation (i.e. we are in a tournament or there is a discrepancy without it)
                total_calc = main_pot + sum(side_pots)
                winnings = sum(safe_decimal(item[1]) for item in self.collected)
                rake = safe_decimal(self.rake)
                target_total = winnings + rake
                if kind != "cash" or abs(target_total - total_calc) > Decimal("0.05"):
                    if hasattr(self.pot, "common") and self.pot.common:
                        main_pot += safe_decimal(sum(self.pot.common.values()))
                    if hasattr(self.pot, "stp") and self.pot.stp:
                        main_pot += safe_decimal(self.pot.stp)

        # Enforce strict monetary invariant: total_pot must equal winnings + rake.
        # We auto-balance the main_pot to equal (winnings + rake) - sum(side_pots) if the difference is significant (> 0.05) but small enough to be a parsing error (<= 5.00), or if it is a tournament.
        # This resolves any minor rounding/dead-blind/ante parsing drifts in both cash and tour hands.
        winnings = sum(safe_decimal(item[1]) for item in self.collected)
        rake = safe_decimal(self.rake)
        target_total = winnings + rake
        total_calc = main_pot + sum(side_pots)
        diff = abs(target_total - total_calc)
        if target_total > 0:
            if kind != "cash" or (diff > Decimal("0.05") and diff <= Decimal("5.00")):
                main_pot = target_total - sum(side_pots)

        # Uncalled returns
        uncalled_returns = []
        if hasattr(self.pot, "returned") and self.pot.returned:
            for p_name, r_val in self.pot.returned.items():
                if r_val:
                    uncalled_returns.append({
                        "player": p_name,
                        "amount": safe_decimal(r_val)
                    })
        ...
        for item in self.collected:
            ...
            winners_list.append({
                "player": item[0],
                "amount": safe_decimal(item[1])
            })

        # Construct final dict
        res = {
            "schema_version": schema_version,
            "room": self.sitename.lower() if self.sitename else "",
            "site_id": self.siteId,
            "hand_id": str(self.handid),
            "game": game_dict,
            "format": format_dict,
            "datetime_utc": dt_str,
            "button_seat": self.buttonpos,
            "players": players_list,
            "blinds": blinds_list,
            "antes": antes_list,
            "hole_cards": hole_cards_dict,
            "streets": streets_active,
            ...
            "rake": self.rake if self.rake is not None else Decimal(0),
            "winners": winners_list
        }

        # Check for Run It Twice
        if self.runItTimes > 1:
            res["rit"] = {
                "enabled": True,
                "runs": self.runItTimes
            }
