...
class Merge(HandHistoryConverter):
    ...
    # Static regexes
    re_identify = re.compile(r'<game\sid="[0-9]+\-[0-9]+"\sstarttime')
    re_SplitHands = re.compile(r"</game>\n+(?=<)")
    re_TailSplitHands = re.compile(r"(</game>)")
    re_GameInfo = re.compile(
        r'<description type="(?P<GAME>Holdem|Omaha|Omaha|Omaha\sH/L8|2\-7\sLowball|A\-5\sLowball|Badugi|5\-Draw\sw/Joker|5\-Draw|7\-Stud|7\-Stud\sH/L8|5\-Stud|Razz|HORSE|RASH|HA|HO|SHOE|HOSE|HAR)(?P<TYPE>\sTournament)?" stakes="(?P<LIMIT>(No Limit|Limit|Pot Limit|Half Pot Limit)\s?)(\sLevel\s\d+\sBlinds)?(\s\(?\$?(?P<SB>[.0-9]+)?/?\$?(?P<BB>[.0-9]+)?(?P<blah>.*)\)?)?"(\sversion="\d+")?\s?/>\s?',
        re.MULTILINE,
    )
    ...
    re_HandInfo = re.compile(
        r'<game id="(?P<HID1>[0-9]+)-(?P<HID2>[0-9]+)" starttime="(?P<DATETIME>.+?)" numholecards="[0-9]+" gametype="[0-9]+" (stakes=".*" )?(multigametype="(?P<MULTIGAMETYPE1>\d+)" )?(seats="(?P<SEATS>[0-9]+)" )?realmoney="(?P<REALMONEY>(true|false))" (multigametype="(?P<MULTIGAMETYPE2>\d+)" )?(data="[0-9]+[|:](?P<TABLENAME>[^|:]+)[|:](?P<TDATA>[^|:]+)[|:]?)?.*>',
        re.MULTILINE,
    )
    ...
    re_Board = re.compile(
        r'<cards type="COMMUNITY" cards="(?P<CARDS>[^"]+)"',
        re.MULTILINE,
    )
    re_Buyin = re.compile(
    ...
    # The named combination lives on the <winner> element (per seat), and a
    # player may win both the high and the low half (two entries).
    re_WinnerHand = re.compile(
        r'<winner amount="[.0-9]+" uncalled="(?:false|true)" potnumber="[0-9]+" player="(?P<PSEAT>[0-9])"[^>]*?hand="(?P<STRING>[^"]+)"',
        re.MULTILINE,
    )
        ...
        if gametype is None:
            ...
            if gametype is None:
                if not re.search("<description", whole_file):
                    msg = "Partial hand history: No <desription> tag"
    ...
    def determineGameType(self, handText):
        ...
        if mg["blah"] is not None and self.re_secondGame.search(mg["blah"]):
            ...
            if fulltable is not True:
                ...
                for seatno in list(acted.keys()):
                    if seatno not in seated:
                        # A seat that acts (e.g. posts the big blind) but is missing
                        # from the <players> preamble can't be reconstructed — its
                        # name and stack are unknown. Skip the hand cleanly instead
                        # of failing with a hard parse error.
                        msg = f"{hand.handid}: seat {seatno} acts but is not in the player preamble"
                        raise FpdbHandPartial(msg)
                ...
                for action in m:
                    ...
                    if sb is None:
                        if action.group("BET") and action.group("BET") != "0.00":
                            ...
                            # log.error((("MergeToFpdb.readBlinds: Cannot calcualte tourney all-in blind for hand '%s'")) % hand.handid)
                            # raise FpdbParseError
    ...
    def mergeMultigametypes(self, handText) -> None:
        ...
        if multigametype:
            try:
                ...
                tmp = handText[0:200]
                log.exception(f"Multigametypes has no lookup for '{multigametype}'")
                raise FpdbParseError
    ...
    def readShownCards(self, hand) -> None:
        # Collect the named combination(s) per seat from the <winner> elements,
        # joining the high/low halves of a split pot.
        combos: dict[str, list[str]] = {}
        for w in self.re_WinnerHand.finditer(hand.handText):
            combos.setdefault(w.group("PSEAT"), []).append(w.group("STRING"))
        ...
        for m in self.re_ShownCards.finditer(hand.handText):
            if m.group("CARDS") is not None:
                ...
                if m.group("SHOWED") == "SHOWN":
                    shown = True
                elif m.group("SHOWED") == "MUCKED":
                    mucked = True

                seat = m.group("PSEAT")
                combo = " | ".join(combos.get(seat, [])) or None

                # print "DEBUG: hand.addShownCards(%s, %s, %s, %s)" %(cards, m.group('PNAME'), shown, mucked)
                hand.addShownCards(
                    cards=cards,
                    player=self.playerNameFromSeatNo(seat, hand),
                    shown=shown,
                    mucked=mucked,
                    string=combo,
                )
    ...
    def determineErrorType(self, hand, function) -> None:
        ...
        if m:
            message = "Found LEAVE. Player left table before hand completed"
        ...
        if message:
            msg = f"Partial hand history: {function} '{hand.handid}' {message}"
    ...
    def readSTP(self, hand) -> None:
        """Read Splash the Pot - not implemented for Merge."""
        pass

    def readTourneyResults(self, hand) -> None:
        """Read tourney results - not implemented for Merge."""
        pass

    def readOther(self, hand) -> None:
        """Read other info - not implemented for Merge."""
        pass
