...
class Betfair(HandHistoryConverter):
    ...
    def compilePlayerRegexs(self, hand: Any) -> None:
        ...
        if not players <= self.compiledPlayers:  # x <= y means 'x is subset of y'
            ...
            self.re_showdown_action = re.compile(
                rf"^{player_re} shows \[ (?P<CARDS>.*?) \](?:[ \t]+(?P<STRING>.+))?$",
                re.MULTILINE,
            )
    ...
    def readShowdownActions(self, hand: Any) -> None:
        ...
        for shows in self.re_showdown_action.finditer(hand.handText):
            ...
            cards = cards.split(", ")
            combo = shows.group("STRING") if "STRING" in shows.groupdict() else None
            hand.addShownCards(cards, shows.group("PNAME"), string=combo.strip() if combo else None)
    ...
    def readOther(self, hand: Any) -> None:
        """Read other info - not implemented for Betfair."""
        pass
