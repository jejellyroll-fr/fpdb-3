...
class Everleaf(HandHistoryConverter):
    ...
    # Static regexes
    re_Identify = re.compile("\\*{5}\\sHand\\shistory\\sfor\\sgame\\s#\\d+\\s|Partouche\\sPoker\\s")
        ...
        if info["limitType"] == "fl" and info["bb"] is not None:
            if info["type"] == "ring":
                try:
                    ...
                    tmp = handText[0:200]
                    log.error(
                        _("EverleafToFpdb.determineGameType: Lim_Blinds has no lookup for '%s' - '%s'")
                        % (mg["BB"], tmp)
                    )
                    raise FpdbParseError
            else:
    ...
    def readHandInfo(self, hand):
        ...
        if m.group("MAX"):
            hand.maxseats = int(
                m.group("MAX")
            )  # assume 4-max unless we have proof it's a larger/smaller game, since everleaf doesn't give seat max info
    ...
    def readShownCards(self, hand):
        ...
        for m in self.re_ShowdownAction.finditer(hand.handText):
            if m.group("CARDS") is not None:
                ...
                if m.group("SHOWED") in ("shows", "showed"):
                    shown = True
