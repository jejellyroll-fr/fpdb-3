...
class TourneySummary:
    ...
    def summaries_from_excel(filenameXLS, tourNoField):
        ...
        for row in rows:
            ...
            data["header"] = header
            # The tournament-id column name varies by export; skip the sheet
            # gracefully instead of raising KeyError when it is absent.
            tour_no = data.get(tourNoField)
            if tour_no and len(tour_no) > 0:
    ...
    def readFile(self, filename):
        # A UTF-16 byte-order mark is unambiguous, so honour it directly. Otherwise
        # cp1252/utf-16 in the codepage list can mis-decode the file (utf-16 turns
        # even-length ASCII into mojibake, cp1252 swallows utf-16 bytes as garbage).
        try:
