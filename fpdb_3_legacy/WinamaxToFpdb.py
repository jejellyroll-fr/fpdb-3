...
class Winamax(HandHistoryConverter):
    ...
    def determineGameType(self, hand_text: str) -> dict[str, Any]:
        ...
        # Winamax does not support split (run-it-twice) games
        info["split"] = False
    ...
    def readPlayerStacks(self, hand: Hand) -> None:
        ...
        for a in m:
            ...
            # Look for the name in actions or summary
            if re.search(r"(\n|^)" + pname_esc + r" (posts|folds|calls|raises|checks|bets|collected|won|shows)", relevant_text) or \
               re.search(r"Dealt to " + pname_esc, relevant_text):
                if plist.get(pname) is None:
                    hand.addPlayer(int(a.group("SEAT")), pname, a.group("CASH"))
                    plist[pname] = [int(a.group("SEAT")), a.group("CASH")]
            else:
                log.debug("readPlayerStacks: Skipping inactive player %s", pname)
        ...
        if len(plist.keys()) < self.MIN_PLAYERS:
            if len(plist.keys()) == 0:
                ...
                msg = f"Less than {self.MIN_PLAYERS} participating players in hand! {hand.handid}."
