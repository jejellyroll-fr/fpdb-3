...
class Winning(HandHistoryConverter):
    ...
    # Anchor the bare "Hand #" / "Game Hand #" header to the start of a line so it
    # does not also match other rooms whose header is "<Room> Hand #..." (e.g.
    # Unibet's "Unibet Hand #1558006027 - ...").
    re_identify = re.compile(r"Game\sID:\s\d+|^(?:Game\s)?Hand\s\#\d+\s\-\s", re.MULTILINE)
    ...
    re_ShowdownMuck1 = re.compile(
        r"^Player\s{PLYR}\smucks\scards".format(**substitutions),
        re.MULTILINE | re.VERBOSE,
    )
    re_ShowdownMuck2 = re.compile(
        r"^(?!Player\s){PLYR}\smucks\s(hand|cards)".format(**substitutions),
        re.MULTILINE | re.VERBOSE,
    )
    re_ShowdownMuck3 = re.compile(
        r"^Player\s{PLYR}\smucks\shand".format(**substitutions),
        re.MULTILINE | re.VERBOSE,
    )
    re_ShowdownShow = re.compile(
        r"^{PLYR}\sshows\s\[(?P<CARDS>.+?)\]".format(**substitutions),
        re.MULTILINE | re.VERBOSE,
    )
    re_ShowdownMuckWithCards = re.compile(
        r"^Player\s{PLYR}\smucks\s\(does\snot\sshow:\s\[(?P<CARDS>.+?)\]\)".format(**substitutions),
        re.MULTILINE | re.VERBOSE,
    )
    ...
    def _markStreets1(self, hand) -> None:
        ...
        if hand.gametype["base"] in ("hold"):
            ...
            m = re.search(
                # Only treat "*** FLOP ***:" as a real flop when board cards follow
                # it ("*** FLOP ***: ["). Winning emits a bare "*** FLOP ***:" header
                # for the pre-deal section, so a hand that ends before the flop must
                # keep its actions on PREFLOP rather than a phantom FLOP street.
                r"(?P<PREFLOP>.+(?=\*\*\* FLOP \*\*\*: \[)|.+)"
    ...
    def _readCommunityCards1(self, hand, street) -> None:
        ...
        if m:
            ...
            # Winning labels the pre-deal section "*** FLOP ***:" even when the
            # hand ends before a flop is dealt, so this street can legitimately
            # carry no board cards. Leave it empty instead of failing the hand.
            log.debug(f"No community cards on {street}, Hand ID: {hand.handid} (hand ended before this street)")
    ...
    def _readCommunityCards2(self, hand, street) -> None:
        ...
        if m:
            ...
            # A street marker can be present with no board cards when the hand
            # ended before that street was dealt. Leave it empty rather than
            # failing the whole hand.
            log.debug(f"No community cards on {street}, Hand ID: {hand.handid} (hand ended before this street)")
    ...
    def _readAntes1(self, hand) -> None:
        ...
        for player in m:
            ...
            if pname not in (p[1] for p in hand.players):
                used_seats = [p[0] for p in hand.players]
                seat = 1
                while seat in used_seats:
                    seat += 1
                hand.addPlayer(seat, pname, "0.00")
    ...
    def readAction(self, hand, street) -> None:
        ...
        # Process uncalled bets if present on this street
        if m := self.re_Uncalled.search(hand.streets[street]):
            uncalled_player = m.group("PNAME").strip()
            if uncalled_player not in (p[1] for p in hand.players):
                used_seats = [p[0] for p in hand.players]
                seat = 1
                while seat in used_seats:
                    seat += 1
                hand.addPlayer(seat, uncalled_player, "0.00")
            uncalled_amount = self.clearMoneyString(m.group("BET"))
    ...
    def _readAction1(self, hand, street) -> None:
        ...
        for action in m:
            ...
            if player is None or player.strip() == "":
                player = " "
            if player not in (p[1] for p in hand.players):
                used_seats = [p[0] for p in hand.players]
                seat = 1
                while seat in used_seats:
                    seat += 1
                hand.addPlayer(seat, player, "0.00")
    ...
    def _readCollectPot2(self, hand) -> None:
        ...
        except Exception as e:  # intentional broad catch: pot-collection parse logs then re-raises
    ...
    def readShowdownActions(self, hand) -> None:
        ...
        # Shows
        for m in self.re_ShowdownShow.finditer(hand.handText):
            if m.group("CARDS"):
                cards = m.group("CARDS").split(" ")
                player = m.group("PNAME")
                hand.addShownCards(
        ...
        # Mucks with cards (from summary or elsewhere)
        for m in self.re_ShowdownMuckWithCards.finditer(hand.handText):
            if m.group("CARDS"):
                cards = m.group("CARDS").split(" ")
                player = m.group("PNAME")
                hand.addShownCards(
        ...
        # Mucks without cards (record the action/status)
        for regex in (
            self.re_ShowdownMuck1,
            self.re_ShowdownMuck2,
            self.re_ShowdownMuck3,
        ):
            for m in regex.finditer(hand.handText):
                player = m.group("PNAME")
                # Using empty list for cards to signify muck without cards known
                # addShownCards adds player to hand.mucked
                hand.addShownCards(cards=[], player=player, shown=False, mucked=True)
