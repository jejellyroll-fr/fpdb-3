...
class Fulltilt(HandHistoryConverter):
    ...
    Lim_Blinds = {
        ...
        "1000": ("250.00", "500.00"),
        "2000.00": ("500.00", "750.00"),
        "2000": ("500.00", "1000.00"),
        "3000.00": ("750.00", "1500.00"),
        "3000": ("750.00", "1500.00"),
        "4000.00": ("1000.00", "2000.00"),
        "4000": ("1000.00", "2000.00"),
        "6000.00": ("1500.00", "3000.00"),
        "6000": ("1500.00", "3000.00"),
    }

    SnG_Fee = {
        50: {"Hyper": 5, "Turbo": 6, "Normal": 7},
    ...
    # Split on a blank-line run that precedes a hand header. A bare "\n\n\n+"
    # both misses exports that separate hands with only "\n\n" (e.g. the 10-game
    # mix sample, which collapses into one 84-marker blob) and can split a single
    # hand on an internal blank-line run. Anchoring on the following header keeps
    # real boundaries only.
    re_SplitHands = re.compile(r"\n\n+(?=(?:Full\sTilt\sPoker\sGame\s\#|FullTiltPoker\sGame\s\#))")
    ...
    re_DateTime = re.compile(
        """((?P<H>[0-9]+):(?P<MIN>[0-9]+):(?P<S>[0-9]+)\\s(?P<TZ>\\w+)\\s-\\s(?P<Y>[0-9]{4})\\/(?P<M>[0-9]{2})\\/(?P<D>[0-9]{2})|(?P<H2>[0-9]+):(?P<MIN2>[0-9]+)\\s(?P<TZ2>\\w+)\\s-\\s\\w+\\,\\s(?P<M2>\\w+)\\s(?P<D2>\\d+)\\,\\s(?P<Y2>[0-9]{4}))(?P<PARTIAL>\\s\\(partial\\))?""",
        re.MULTILINE,
    )
        ...
        if info["limitType"] == "fl" and info["bb"] is not None:
            if info["type"] == "ring":
                try:
                    ...
                    tmp = handText[0:200]
                    log.error(
                        _("FulltiltToFpdb.determineGameType: Lim_Blinds has no lookup for '%s' - '%s'") % (bb, tmp)
                    )
                    raise FpdbParseError
            else:
    ...
    def readHandInfo(self, hand):
        ...
        if m.group("TOURNAMENT") is not None:
            ...
            if n.group("BUYINGUAR") is not None and n.group("FEE") is not None:
                ...
                if (
                    hand.maxseats == 2
                    and buyinfee in self.HUSnG_Fee
                    and self.HUSnG_Fee[buyinfee].get(hand.speed) is not None
                ):
                    hand.fee = self.HUSnG_Fee[buyinfee][hand.speed]
                    hand.buyin = buyinfee - hand.fee
                if (
                    hand.maxseats != 2
                    and buyinfee in self.SnG_Fee
                    and self.SnG_Fee[buyinfee].get(hand.speed) is not None
                ):
    ...
    def markStreets(self, hand):
        if hand.gametype["base"] == "hold":
            m = re.search(
                r"\*\*\* HOLE CARDS \*\*\*(?P<PREFLOP>(.+\*\*\* FLOPET \*\*\* (?P<FLOPET>\[\S\S\]))?.+(?=\*\*\* FLOP (1\s)?\*\*\*)|.+)"
                ...
                r"(\*\*\* RIVER 2 \*\*\* \[\S\S \S\S \S\S \S\S] (?P<RIVER2>\[\S\S\].+))?",
                hand.handText,
                re.DOTALL,
            )
            ...
            if hand.gametype["category"] != "5_studhi":
                ...
                m = re.search(
                    r"(?P<ANTES>.+(?=\*\*\* 2ND STREET \*\*\*)|.+)"
                    r"(\*\*\* 2ND STREET \*\*\*(?P<SECOND>.+(?=\*\*\* 3RD STREET \*\*\*)|.+))?"
                    r"(\*\*\* 3RD STREET \*\*\*(?P<THIRD>.+(?=\*\*\* 4TH STREET \*\*\*)|.+))?"
                    r"(\*\*\* 4TH STREET \*\*\*(?P<FOURTH>.+(?=\*\*\* 5TH STREET \*\*\*)|.+))?"
                    r"(\*\*\* 5TH STREET \*\*\*(?P<FIFTH>.+))?",
                    hand.handText,
                    re.DOTALL,
                )
        elif hand.gametype["base"] in ("draw"):
            if "*** HOLE CARDS ***" not in hand.handText:
                # No cards were ever dealt (e.g. everyone sat out before the
                # deal): a dead hand. Skip it instead of erroring downstream.
                raise FpdbHandPartial(_("Hand '%s' was not dealt (dead hand).") % hand.handid)
            m = re.search(
                r"(?P<PREDEAL>.+(?=\*\*\* HOLE CARDS \*\*\*)|.+)"
    ...
    def readHoleCards(self, hand):
        ...
        for street, text in hand.streets.items():
            ...
            for found in m:
                ...
                if (
                    (hand.gametype["category"] == "5_studhi" and street == "SECOND" and len(oldcards) == 1)
                    or hand.gametype["category"] != "5_studhi"
                    and street == "THIRD"
                    and len(oldcards) == 2
                ):  # hero in stud game
                    hand.hero = player
        ...
        for action in m:
            if action.group("ATYPE") == " folds":
                ...
                log.debug(
                    _("DEBUG:")
                    + " "
                    + _("Unimplemented %s: '%s' '%s'") % ("readAction", action.group("PNAME"), action.group("ATYPE"))
                )
    ...
    def getTableTitleRe(type, table_name=None, tournament=None, table_number=None):
        ...
        if type == "tour":
            regex = (
                "Tournament " + re.escape(str(tournament)) + "( - Entry \\d+)?, Table " + re.escape(str(table_number))
            )
        else:
    ...
    def readTourneyResults(self, hand):
        """Read tourney results - not implemented."""
        pass

    def readOther(self, hand):
        """Read other info - not implemented."""
        pass

    def readSummaryInfo(self, summaryInfoList):
        """Read summary info - not implemented."""
        return True
