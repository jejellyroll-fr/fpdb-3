...
class PokerStars(HandHistoryConverter):
    ...
    re_game_info = re.compile(
        """
          (?P<SITE>PokerStars|POKERSTARS|888Poker|Hive\\sPoker|Full\\sTilt|PokerMaster|Run\\sIt\\sOnce\\sPoker|BetOnline|PokerBros|MPLPoker|SupremaPoker)(?P<TITLE>\\sGame|\\sHand|\\sHome\\sGame|\\sHome\\sGame\\sHand|Game|\\s(Zoom|Rush)\\sHand|\\sGAME|\\s\\#\\sVersion:)\\s\\#?(?P<HID>[0-9]+):?\\s+
    ...
    # Final board listed in the SUMMARY, used to recover a community card that
    # PokerStars occasionally omits on the street line (e.g. "*** TURN *** [...] []").
    re_board_summary = re.compile(r"^Board \[(?P<CARDS>[^\]]+)\]", re.MULTILINE)
    ...
    try:
        re_action = re.compile(
            r"""{PLYR}:(?P<ATYPE>\sbets|\schecks|\sraises|\scalls|\sfolds|\sdiscards|\sstands\spat)(\s{CUR}(?P<BET>[,.\d]+))?(\sto\s{CUR}(?P<BETTO>[,.\d]+))?\s*(and\sis\sall.in)?(and\shas\sreached\sthe\s[{LSC}\d\.,]+\scap)?(\son|\scards?)?(\s\(disconnect\))?(\s\[(?P<CARDS>.+?)\])?\s*$""".format(
                **substitutions,
            ),
            re.MULTILINE | re.VERBOSE,
        )
    ...
    def processPotMatch(self, hand, m, is_cashout=False):
        pname = m.group("PNAME")
        pot = m.group("POT")
        pot = pot.replace(",", "")
        ...
        if is_cashout:
            pot_type = "cashout"
        elif "side pot" in m.group(0):
            pot_type = "side"

        # Add to hand.pots
        # Logic to append to correct pot list...
        # Legacy parser handles winning logic inside here often via hand.addWonPot
        # but we need to ensure cashout isn't treated as "won hand" flag later.
    ...
    def markStreets(self, hand: Hand) -> None:
        ...
        if hand.gametype["category"] in ("27_1draw", "fivedraw") and "*** DRAW ***" not in hand.handText:
            # Stars single-draw hands have no marker between the deal and the draw,
            # which upsets the accounting and incorrectly sets HandsPlayers.cardxx
            # (and the replayer never updates the hand after the draw). Insert a
            # "*** DRAW ***" marker before the FIRST discard/stand-pat line so the
            # draw round becomes its own street. The previous greedy split matched
            # from the start of the text and prepended the marker uselessly.
            hand.handText = re.sub(
                r"(?m)^([^\n]*?(?: stands pat|: discards))",
                r"*** DRAW ***\n\1",
                hand.handText,
                count=1,
    ...
    def readCommunityCards(
        ...
        if self.re_empty_card.search(hand.streets[street]):
            # PokerStars sometimes omits a community card on the street line but
            # still lists the full board in the SUMMARY — recover it from there.
            recovered = self._recover_board_from_summary(hand, street)
            if recovered:
                hand.setCommunityCards(street, recovered)
                log.info("Recovered blank %s card from SUMMARY board: %s", street, recovered)
    ...
    def _recover_board_from_summary(self, hand: Hand, street: str):
        ...
        m = self.re_board_summary.search(hand.handText)
        if not m:
            return None
    ...
    def readHoleCards(self, hand: Hand) -> None:
        ...
        for street in ("PREFLOP", "DEAL"):
            if street in hand.streets:
                ...
                # Some PokerStars 5-card-draw hands have no "*** DRAW ***" separator,
                # so the post-draw "Dealt to X [kept] [drawn]" line lands in the DEAL
                # section next to the initial deal. Capture the first (initial) deal on
                # this street and route any later draw lines (those with OLDCARDS) to the
                # following draw streets, so they don't clobber the original starting hand.
                draw_streets = [
                    s for s in getattr(hand, "holeStreets", []) if s not in ("PREFLOP", "DEAL")
                ]
                next_draw = 0
                seen_initial = False
                for found in m:
                    hand.hero = found.group("PNAME")
                    if "cards" in found.group("NEWCARDS"):
                        continue
                    newcards = found.group("NEWCARDS").split(" ")
                    oldcards = found.group("OLDCARDS")
                    if oldcards is None and not seen_initial:
                        # Genuine initial deal.
                        hand.addHoleCards(
                            street,
                            hand.hero,
                            closed=newcards,
                        ...
                        # An embedded draw line: store the kept/drawn cards on the
                        # appropriate draw street instead of overwriting the deal.
                        target = draw_streets[min(next_draw, len(draw_streets) - 1)]
                        next_draw += 1
                        hand.addHoleCards(
                            target,
                            hand.hero,
                            open=newcards,
                            closed=oldcards.split(" ") if oldcards else [],
                            shown=False,
                            mucked=False,
                            dealt=False,
                        )
                    else:
                        # No draw street available (e.g. hold'em): keep prior behaviour.
                        hand.addHoleCards(
                            street,
                            hand.hero,
                            closed=newcards,
    ...
    def _processProgressiveBounties(self, hand: Hand) -> None:
        ...
        for a in self.re_progressive.finditer(hand.handText):
            if a.group("PNAME") not in ko_amounts:
                ko_amounts[a.group("PNAME")] = Decimal("0")
            ko_amounts[a.group("PNAME")] += 100 * Decimal(a.group("AMT"))
    ...
    def _recordTourneyResult(
        ...
        """Store a tournament result found in a hand history.

        PokerStars often writes only elimination lines in the hand history, and
        keeps the monetary result in a separate tournament summary. Keep the rank
        even when winnings are unknown so tournament wins/losses are not silently
        lost during hand imports.
        """
        hand.tourneyRanks[player_name] = rank
    ...
    def _readTourneyRanks(self, hand: Hand) -> None:
        ...
        if not isinstance(getattr(hand, "tourneyRanks", None), dict):
            hand.tourneyRanks = {}
        if not isinstance(getattr(hand, "tourneyWinnings", None), dict):
            hand.tourneyWinnings = {}
        if not isinstance(getattr(hand, "tourneyWinningsCurrency", None), dict):
            hand.tourneyWinningsCurrency = {}

        for match in self.re_winning_rank_one.finditer(hand.handText):
            self._recordTourneyResult(
                hand,
                match.group("PNAME"),
                1,
                Decimal(self.clearMoneyString(match.group("AMT"))),
                hand.gametype.get("currency"),
            )

        for match in self.re_winning_rank_other.finditer(hand.handText):
            self._recordTourneyResult(
                hand,
                match.group("PNAME"),
        ...
        for match in self.re_rank_other.finditer(hand.handText):
            self._recordTourneyResult(hand, match.group("PNAME"), int(match.group("RANK")))

        # Heads-up finish lines usually name only the runner-up. If the current
        # hand has exactly one remaining unranked player, that player won.
        if 2 in hand.tourneyRanks.values() and 1 not in hand.tourneyRanks.values():
            current_players = [player[1] for player in hand.players]
            unranked_players = [player for player in current_players if player not in hand.tourneyRanks]
    ...
    def _calculateBovadaAdjustments(self, hand: Hand) -> tuple[bool, bool, Decimal, Decimal]:
        ...
        Returns:
        tuple[bool, bool, Decimal, Decimal]: Flags and adjustment values for Bovada walk scenarios.
        """
        acts = hand.actions.get("PREFLOP")
        bovada_uncalled_v1, bovada_uncalled_v2 = False, False
        blindsantes, adjustment = Decimal("0"), Decimal("0")
    ...
    def _addCollectPotWithAdjustment(
        ...
        Args:
            ...
            adjustments (tuple[bool, bool, Decimal, Decimal]): Flags and adjustment values for Bovada walk scenarios.
        ...
        if bovada_uncalled_v1 and Decimal(pot) == (Decimal(blindsantes) + Decimal(hand.pot.stp)):
            hand.addCollectPot(player=player, pot=str(Decimal(pot) - adjustment))
        elif bovada_uncalled_v2:
            hand.addCollectPot(player=player, pot=str(Decimal(pot) * 2))
        else:
    ...
    def _addCashOutPotWithAdjustment(
        ...
        player = match["PNAME"]
        if bovada_uncalled_v1 and Decimal(pot) == (Decimal(blindsantes) + Decimal(hand.pot.stp)):
            hand.addCashOutPot(player=player, pot=str(Decimal(pot) - adjustment))
        elif bovada_uncalled_v2:
            hand.addCashOutPot(player=player, pot=str(Decimal(pot) * 2))
        else:
    ...
    def readCollectPot(self, hand: Hand) -> None:
        ...
        # Players who cashed out: PokerStars does not award them the showdown
        # pot ("pot not awarded as player cashed out"), so the cash-out amount
        # is their only collection. Counting the SUMMARY "won (...)" line on top
        # of the cash-out double-counts their winnings (GitHub issue #98).
        cashed_out_players = {m.group("PNAME") for m in self.re_collect_pot3.finditer(hand.handText)}

        processed_cashout_matches = False
        ...
        if hand.runItTimes == 0:
            ...
            if self.re_collect_pot3.search(hand.handText):
                for m in self.re_collect_pot3.finditer(hand.handText):
                    self._addCashOutPotWithAdjustment(hand, m, adjustments)
                    i += 1
                    processed_cashout_matches = True
            for m in self.re_collect_pot.finditer(post):
                if m.group("PNAME") in cashed_out_players:
                    log.info(
                        "Skipping SUMMARY collection for cashed-out player %s (pot not awarded)",
                        m.group("PNAME"),
                    )
                    continue
        ...
        for m in self.re_collect_pot3.finditer(pre):
            ...
            if not processed_cashout_matches:
                self._addCashOutPotWithAdjustment(hand, m, adjustments)
                i += 1
