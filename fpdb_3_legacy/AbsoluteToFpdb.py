...
class Absolute(HandHistoryConverter):
    ...
    # Static regexes
    re_Identify = re.compile("Stage\\s+\\#C?[0-9]+")
    ...
    re_HorseGameInfo = re.compile(r"^Game Type: (?P<LIMIT>Limit) (?P<GAME>Holdem)", re.MULTILINE)
    ...
    def compilePlayerRegexs(self, hand):
        ...
        if not players <= self.compiledPlayers:  # x <= y means 'x is subset of y'
            ...
            self.re_PostBoth = re.compile(
                r"^%s - Posts (dead )?(?:\$| €|)(?P<BB>[,.0-9]+) (dead )?(?:\$| €|)(?P<SB>[,.0-9]+)" % player_re,
                re.MULTILINE,
            )
            self.re_Action = re.compile(
                r"^%s - (?P<ATYPE>Bets |Raises |All-In |All-In\(Raise\) |Calls |Folds|Checks)?\$?(?P<BET>[,.0-9]+)?"
                % player_re,
                re.MULTILINE,
            )
            self.re_ShowdownAction = re.compile(
                r"^%s - Shows \[(?P<CARDS>.*)\] \((?P<STRING>.+?)\)" % player_re, re.MULTILINE
            )
            self.re_CollectPot = re.compile(
                r"^Seat [0-9]: %s(?: \(dealer\)|)(?: \(big blind\)| \(small blind\)|) (?:won|collected) Total \((?:\$| €|)(?P<POT>[,.0-9]+)\)(.*72 Prop Win \((?:\$| €|)(?P<PROP>[,.0-9]+)\))?"
                % player_re,
                re.MULTILINE,
            )
        ...
        if info["limitType"] == "fl" and info["bb"] is not None:
            if info["type"] == "ring":
                try:
                    ...
                    tmp = handText[0:200]
                    log.error(
                        _("AbsoluteToFpdb.determineGameType: Lim_Blinds has no lookup for '%s' - '%s'")
                        % (info["bb"], tmp)
                    )
                    raise FpdbParseError
        return info
    ...
    def readHandInfo(self, hand):
        ...
        # Absolute headers carry the source TZ (e.g. "(ET)"); default to ET when absent.
        hand.startTime = HandHistoryConverter.changeTimezone(
            hand.startTime, m.group("TZ") or "ET", "UTC"
        )
