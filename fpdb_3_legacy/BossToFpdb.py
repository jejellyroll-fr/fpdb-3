...
class Boss(HandHistoryConverter):
    ...
    re_Button = re.compile(
        '<ACTION TYPE="HAND_DEAL" PLAYER="(?P<BUTTON>[^"]+)">\n<CARD LINK="[0-9b]+"></CARD>\n<CARD LINK="[0-9b]+"></CARD></ACTION>\n<ACTION TYPE="ACTION_',
        re.MULTILINE,
    )
    re_PlayerInfo = re.compile(
        '^<PLAYER NAME="(?P<PNAME>.+)" SEAT="(?P<SEAT>[0-9]+)" AMOUNT="(?P<CASH>[.0-9]+)"( STATE="(?P<STATE>STATE_EMPTY|STATE_PLAYING|STATE_SITOUT)" DEALER="(Y|N)")?></PLAYER>',
        re.MULTILINE,
    )
        ...
        if info["limitType"] == "fl" and info["bb"] is not None:
            if info["type"] == "ring":
                try:
                    ...
                    tmp = handText[0:200]
                    log.error(
                        _("BossToFpdb.determineGameType: Lim_Blinds has no lookup for '%s' - '%s'") % (mg["BB"], tmp)
                    )
                    raise FpdbParseError
            else:
    ...
    def markStreets(self, hand):
        ...
        if hand.gametype["base"] in ("hold"):
            m = re.search(
                '<ACTION TYPE="HAND_BLINDS" PLAYER=".+" KIND="(HAND_BB|HAND_SB)" VALUE="[.0-9]+"></ACTION>(?P<PREFLOP>.+(?=<ACTION TYPE="HAND_BOARD" VALUE="BOARD_FLOP")|.+)'
                ...
                '((?P<RIVER><ACTION TYPE="HAND_BOARD" VALUE="BOARD_RIVER" POT="[.0-9]+".+?>.+(?=<SHOWDOWN NAME="HAND_SHOWDOWN")|.+))?',
                hand.handText,
                re.DOTALL,
            )
    ...
    def readBlinds(self, hand):
        ...
        for a in self.re_PostDSB.finditer(hand.handText):
            hand.addBlind(a.group("PNAME"), "secondsb", a.group("DSB"))
    ...
    def convertBossCards(self, card):
        card = int(card)
        retCard = ""
        cardconvert = {1: "A", 10: "T", 11: "J", 12: "Q", 13: "K"}
