...
SITE_DETECTION_ERRORS = (AttributeError, KeyError, OSError, TypeError, ValueError)
    ...
    def _detect_skin(self, skin: str) -> dict[str, Any]:  # noqa: C901, PLR0912
        ...
        if self.platform == "Windows":
            ...
            if skin == "PMU Poker":
                ...
                # Check both "PMU PLAY" (modern) and "PMU Online Poker" (legacy/alternative)
                pmu_container = self._check_path_exists(
                    str(
            ...
            elif skin == "Bwin.fr Poker":
                # Bwin.fr Poker on macOS uses a sandboxed container
                bwin_container = self._check_path_exists(
                    str(
                        Path(containers_path)
                        / "com.entainpt.bwinpokerfr.app"
                        / "Data"
                        / "Library"
                        / "Application Support"
                        / "Bwin.fr Online Poker"
                    ),
                    str(
                        Path(containers_path)
                        / "com.entainpt.bwinpokerfr.app"
                ...
                if bwin_container:
                    base_path = bwin_container
            elif skin == "PartyPoker.fr":
                # PartyPoker.fr on macOS uses a sandboxed container
                party_container = self._check_path_exists(
                    str(
                        Path(containers_path)
                        / "com.entainpt.partypokerfr.app"
                        / "Data"
                        / "Library"
                        / "Application Support"
                        / "PartyPoker.fr Online Poker"
                    ),
                    str(
                        Path(containers_path)
                        / "com.entainpt.partypokerfr.app"
    ...
    def _detect_pokerclient(self) -> dict[str, Any]:  # noqa: C901
        ...
        try:
            ...
            skin_mapping = {
                "pmupoker": "PMU Poker",
                "pmu": "PMU Poker",
                "bwin.fr": "Bwin.fr Poker",
                "bwinfr": "Bwin.fr Poker",
                "partypoker.fr": "PartyPoker.fr",
...
def main(argv=None):
    ...
    except SITE_DETECTION_ERRORS as e:
