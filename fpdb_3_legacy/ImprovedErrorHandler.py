...
class ImprovedErrorHandler:
    ...
    def should_reset_file_position(self, file_path: str, error: ParseError) -> bool:
        ...
        Returns:
            True if file position should be reset (rollback to retry), False otherwise (skip/discard)
        """
        if error.severity == ErrorSeverity.PERMANENT:
            log.info(f"Permanent error detected for {file_path}, maintaining file position (skipping)")
            return False

        if error.severity == ErrorSeverity.TEMPORARY:
            log.debug(f"Temporary error for {file_path}, resetting file position (retrying)")
            return True
        ...
        log.debug(f"Recoverable error for {file_path}, resetting file position to retry (recent errors: {len(recent_errors)})")
        return True
