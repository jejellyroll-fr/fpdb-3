...
class OnGame(HandHistoryConverter):
    ...
    def markStreets(self, hand):
        if hand.gametype["base"] in ("hold"):
            m = re.search(
                r"pocket cards(?P<PREFLOP>.+?(?= Dealing flop )|.+(?=Summary))"
            ...
            m = re.search(
                r"(?P<ANTES>.+(?=Dealing pocket cards)|.+)"
            ...
            if len(hand.handText) == len(discard_split[0]):
                ...
                for i in discard_split:
                    hand.handText += i
            m = re.search(
                r"(?P<PREDEAL>.+(?=Dealing pocket cards)|.+)"
    ...
    def readSTP(self, hand):
        """Read Splash the Pot - not implemented for OnGame."""
        pass

    def readTourneyResults(self, hand):
        """Read tourney results - not implemented for OnGame."""
        pass

    def readOther(self, hand):
        """Read other info - not implemented for OnGame."""
        pass
