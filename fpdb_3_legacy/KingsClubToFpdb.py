...
# KingsClub HH Format
log = get_logger("kings_club_parser")
...
class KingsClub(HandHistoryConverter):
    ...
    re_Action = re.compile(
        r"""^{PLYR}(?P<ATYPE>\sbets|\schecks|\sraises|\scalls|\sfolds|\sdiscards|\sstands\spat|\sdraws)(\s{CUR}(?P<BET>[,.\d]+))?(\sto\s{CUR}(?P<BETTO>[,.\d]+))?\s*(,\sand\sis\sall.in)?(and\shas\sreached\sthe\s\[{CUR}\d\.,]+\scap)?(\son|\scards?)?(\s\(disconnect\))?(\s\[(?P<CARDS>.+?)\]\sdraws\s\[(?P<DRAWS1>.+?)\](\s\[(?P<DRAWS2>.+?)\])?)?\s*$""".format(
            **substitutions
        ),
        re.MULTILINE | re.VERBOSE,
    )
    ...
    def markStreets(self, hand) -> None:
        ...
        if hand.gametype["split"]:
            m = re.search(
                r"\*\*\* HOLE CARDS \*\*\*(?P<PREFLOP>.+(?=\*\*\* BOARD 1 - FLOP \*\*\*)|.+)"
                ...
                r"(\*\*\* BOARD 2 - RIVER \*\*\* \[\S\S \S\S \S\S \S\S] (?P<RIVER2>\[\S\S\].+))?",
                hand.handText,
                re.DOTALL,
            )
            ...
            m = re.search(
                ...
                r"(\*\*\* RIVER \*\*\* \[\S\S \S\S \S\S \S\S] (?P<DRAWTHREE>\[\S\S\].+))?",
                hand.handText,
                re.DOTALL,
            )
        ...
        if hand.gametype["base"] in ("hold") and not hand.gametype["split"]:
            m1 = re.search(
                ...
                r"(\*\*\* BOARD 2 - RIVER \*\*\* \[(?P<FLOP2>(\S\S|\-) (\S\S|\-) (\S\S|\-)) (?P<TURN2>(\S\S|\-))] (?P<RIVER2>\[\S\S\].+))",
                post,
                re.DOTALL,
            )
            if m1:
            ...
            if action.group("ATYPE") == " folds":
                ...
                if action.group("BETTO") is not None:
                    hand.addRaiseTo(
                        ...
                        (
                            str(
                                Decimal(self.clearMoneyString(action.group("BETTO"))) * 100,
                            )
                            if hand.tourNo is not None
                    ...
                    hand.addCallandRaise(
                        ...
                        (
                            str(
                                Decimal(self.clearMoneyString(action.group("BET"))) * 100,
                            )
                            if hand.tourNo is not None
                            else self.clearMoneyString(action.group("BET"))
                        ),
    ...
    def readCollectPot(self, hand) -> None:
        ...
        for m in self.re_CollectPot.finditer(hand.handText):
            pot = (
                str(Decimal(self.clearMoneyString(m.group("POT"))) * 100)
                if hand.tourNo is not None
                else self.clearMoneyString(m.group("POT"))
            )
    ...
    def readOther(self, hand) -> None:
        """Read other info - not implemented for KingsClub."""
        pass
