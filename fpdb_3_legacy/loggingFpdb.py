...
LOG_CONFIG_SAVE_ERRORS = (OSError, TypeError, ValueError)
LOG_CONFIG_LOAD_ERRORS = (OSError, json.JSONDecodeError, TypeError, ValueError)
LOG_CONFIG_APPLY_ERRORS = (
    AttributeError,
    KeyError,
    OSError,
    TypeError,
...
class LogConfig:
    ...
    def save_config(self, registry: LoggerRegistry) -> bool:
        ...
        except LOG_CONFIG_SAVE_ERRORS as e:
    ...
    def load_config_data(self) -> dict:
        ...
        try:
            with open(self.config_file, encoding="utf-8") as f:
                return json.load(f)
        except LOG_CONFIG_LOAD_ERRORS as e:
    ...
    def load_config(self, registry: LoggerRegistry) -> bool:
        ...
        except LOG_CONFIG_APPLY_ERRORS as e:
...
def show_logger_dev_tool(parent=None):
    ...
    Instantiates and displays the LoggerDevTool dialog, returning its result code. If PySide6 is not available, logs an error and returns None.

    Args:
        parent: The parent widget for the dialog, if any.

    Returns:
        Union[int, None]: The result code from the dialog execution, or None if the tool could not be shown.
    """
    try:
...
def auto_load_logger_config() -> None:
    ...
    except Exception as e:  # intentional broad catch: startup logging config is best-effort.
...
def ensure_console_handlers_configured() -> None:
    ...
    except Exception as e:  # intentional broad catch: console handler discovery is best-effort.
...
def setup_logging(log_dir: str | None = None, *, console_only: bool = False) -> None:
    ...
    except Exception:  # intentional broad catch: setup preserves legacy re-raise semantics.  # noqa: TRY203
