...
street4Raises INT,
        ...
        ################################
        if db_server == "mysql":
            self.query["createPlayerAutoNotesTable"] = """CREATE TABLE PlayerAutoNotes (
                        id BIGINT UNSIGNED AUTO_INCREMENT NOT NULL, PRIMARY KEY (id),
                        playerId INT UNSIGNED NOT NULL, FOREIGN KEY (playerId) REFERENCES Players(id),
                        handId BIGINT UNSIGNED NOT NULL, FOREIGN KEY (handId) REFERENCES Hands(id),
...
street1Discards INT,
        ...
        if db_server in {"mysql", "postgresql"}:
            self.query["addBombPotColumn"] = """ALTER TABLE Hands ADD COLUMN bombPot BIGINT DEFAULT 0"""
        elif db_server == "sqlite":
            self.query["addBombPotColumn"] = """ALTER TABLE Hands ADD COLUMN bombPot INT DEFAULT 0"""
        # Add index for bombPot queries
        if db_server == "mysql":
            self.query["addBombPotIndex"] = """ALTER TABLE Hands ADD INDEX bomb_pot_idx (bombPot)"""
        elif db_server in ("postgresql", "sqlite"):
            self.query["addBombPotIndex"] = """CREATE INDEX bomb_pot_idx ON Hands (bombPot)"""
