...
class PokerTrackerSummary(TourneySummary):
    ...
    def getSplitRe(self, _head: str) -> re.Pattern[str]:
        ...
        The importer calls this as ``Class.getSplitRe(Class, head)`` (two
        positional args), the convention shared by every other summary parser,
        so this must be an instance-style method — a one-arg @staticmethod
