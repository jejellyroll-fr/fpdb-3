...
class BetOnline(HandHistoryConverter):
    ...
    re_total_pot: ClassVar[re.Pattern[str]] = re.compile(
        r"^Total pot (?P<POT>[{NUM}]+)( \| Rake (?P<RAKE>[{NUM}]+))?".format(**substitutions),
        re.MULTILINE,
    )
    ...
    re_uncalled: ClassVar[re.Pattern[str]] = re.compile(
        r"^Uncalled bet \((?:{LS})?(?P<BET>[{NUM}]+)\) returned to {PLYR}$".format(**substitutions),
        re.MULTILINE,
    )
    ...
    def readAction(self, hand: Any, street: str) -> None:
        ...
        if m := self.re_uncalled.search(hand.streets[street]):
            uncalled_player = self.unknownPlayer(hand, m.group("PNAME").strip())
    ...
    def readTourneyResults(self, hand: Any) -> None:
        """Read tourney results - not implemented for BetOnline."""
        pass

    def readOther(self, hand: Any) -> None:
        """Read other info - not implemented for BetOnline."""
        pass
