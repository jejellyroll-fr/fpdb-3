...
PARTYPOKER_DATETIME_ERRORS = (TypeError, ValueError)
PARTYPOKER_BLIND_ERRORS = (AttributeError, KeyError, TypeError, ValueError)
PARTYPOKER_DECIMAL_ERRORS = (InvalidOperation, TypeError, ValueError)
...
class PartyPoker(HandHistoryConverter):
    ...
    # lines; split before that marker too, otherwise a whole file collapses into
    ...
    def _processDateTime(self, hand, datetime_str) -> None:
        ...
        except PARTYPOKER_DATETIME_ERRORS as e:
    ...
    def _processHID(self, hand, hid) -> None:
        ...
        if str(hid) == "1111111111":
            ...
            #
            # The per-hand text lives on `hand.handText` in the normal
            # converter path; `self.handText` is the WHOLE file buffer and
            ...
            log.debug(f"Generated deterministic hand ID hand_id: {hand.handid}, round_penny: True")
        else:
    ...
    def _findFirstEmptySeat(self, hand, startSeat):
        ...
        while seat in occupied_seats:
            seat += 1
            if hand.maxseats and seat > hand.maxseats:
                seat = 1
            if seat > 10:
                break
    ...
    def readBlinds(self, hand) -> None:
        ...
        if (
            ...
            except PARTYPOKER_BLIND_ERRORS as e:
            ...
            smartMin = lambda A, B: A if Decimal(A) <= Decimal(B) else B
    ...
    def readAction(self, hand, street) -> None:
        ...
        seated_players = {p[1] for p in hand.players}
        ...
        for action in m:
            ...
            # Some exported PartyPoker hands are partially anonymised: the seats
            # (and blinds) use "PlayerN" placeholders while the action lines keep
            # the real names, so the actor was never seated. Such a hand cannot be
            # reconstructed correctly (the real-to-seat mapping is unknown), so skip
            # it cleanly instead of crashing later with an "unknown player" error.
            if playerName not in seated_players:
                msg = f"{hand.handid}: action by unseated player '{playerName}' (partially anonymised hand)"
                raise FpdbHandPartial(msg)
    ...
    def readTourneyResults(self, hand) -> None:
        ...
        if m:
            ...
            if "AMT" in m.groupdict():
                ...
                except PARTYPOKER_DECIMAL_ERRORS as e:
