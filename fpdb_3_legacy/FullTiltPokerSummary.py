...
class FullTiltPokerSummary(TourneySummary):
    ...
    def parseSummaryFile(self):
        ...
        elif mg["CURRENCY"] == "FPP":
            self.buyinCurrency = "FTFP"
        elif mg["CURRENCY"] == "FTP":
            self.buyinCurrency = "FTFP"
        elif mg["CURRENCY"] == "T$":
            self.buyinCurrency = "USD"
        elif mg["CURRENCY"] == "Play Chips":
    ...
    def setCurrency(self, m, currency=None):
        if m.group("CURRENCY1") == "$":
            currency = "USD"
        elif m.group("CURRENCY1") == "€":
            currency = "EUR"
        elif m.group("CURRENCY2") == "FPP":
            currency = "FTFP"
        elif m.group("CURRENCY2") == "FTP":
            currency = "FTFP"
        elif m.group("CURRENCY2") == "T$":
            currency = "USD"
        elif m.group("CURRENCY2") == "Play Chips":
            currency = "play"
        return currency
