...
CONFIG_ACCESS_ERRORS = (AttributeError, KeyError, TypeError, ValueError)
...
class ConfigurationManager:
    ...
    DYNAMIC_CHANGE_PATHS: ClassVar[list[str]] = [
        "supported_sites.*.enabled",  # Site auto-import enable/disable
    ...
    def initialize(self, config_file=None) -> None:
        ...
        with self._lock:
            ...
            except Exception as e:  # intentional broad catch: initialization wraps legacy Config construction failures.
    ...
    def _requires_restart(self, change: ConfigChange) -> bool:
        ...
        for critical_path in self.RESTART_REQUIRED_PATHS:
            if self._path_matches(change.path, critical_path):
                return True
    ...
    def _path_matches(self, change_path: str, observed_path: str) -> bool:
        """Return True when a concrete config path matches a dotted pattern.

        Patterns may contain ``*`` as a single path-segment wildcard, for
        example ``supported_sites.*.HH_path``.
        """
        if "*" not in observed_path:
            return change_path == observed_path or change_path.startswith(f"{observed_path}.")
        ...
        for change in changes:
            for observer in self._observers:
                ...
                for path in observed_paths:
                    if self._path_matches(change.path, path):
            ...
            try:
                for change in changes_list:
                    ...
                    if not success:
                        ...
                        all_success = False
            except Exception as e:  # intentional broad catch: observer callbacks are plugin/UI-defined boundaries.
