...
class PokerTracker(HandHistoryConverter):
    ...
    re_HandInfo_Tour = re.compile(
        r"""
          ...
          (?P<TOUR>\(Tournament:\s(.+)?\sBuy\-In:\s(?P<BUYIN>(?P<BIAMT>[{LS}\d.,]+)\s?\+?\s?((?P<BIBOUNTY>[{LS}\d.,]+)\s?\+\s?)?(?P<BIRAKE>[{LS}\d.,]+))\))
          """.format(**substitutions),
        re.MULTILINE | re.VERBOSE,
    )
    ...
    re_PostSB = re.compile(
        r"^{PLYR}:? ((posts|posted) the small blind( of)?|(Post )?SB) (\- )?{CUR}(?P<SB>[{NUM}]+)".format(
            **substitutions
        ),
        re.MULTILINE,
    )
    re_PostBB = re.compile(
        r"^{PLYR}:? ((posts|posted) the big blind( of)?|posts the dead blind of|(Post )?BB) (\- )?{CUR}(?P<BB>[{NUM}]+)".format(
            **substitutions
        ),
        re.MULTILINE,
    )
    re_Antes = re.compile(
        r"^{PLYR}:? ((posts|posted) (the )?ante( of)?|(Post )?Ante) (\- )?{CUR}(?P<ANTE>[{NUM}]+)".format(
            **substitutions
        ),
        re.MULTILINE,
    )
    ...
    re_Action1 = re.compile(
        r"""^{PLYR}:?(?P<ATYPE>\sbets|\schecks|\sraises|\scalls|\sfolds|\sBet|\sCheck|\sRaise(\sto)?|\sCall|\sFold|\sAllin)(?P<RAISETO>\s\(NF\))?(\sto)?(\s{CUR}(?P<BET>[{NUM}]+))?\s*(and\sis\sall.in)?(and\shas\sreached\sthe\s\[{CUR}\d\.,]+\scap)?(\son|\scards?)?(\s\[(?P<CARDS>.+?)\])?\s*$""".format(
            **substitutions
        ),
        re.MULTILINE | re.VERBOSE,
    )
    ...
    re_ShownCards1 = re.compile(
        r"^{PLYR}:? (?P<SHOWED>shows|Shows|mucked|mucks|Mucks) \[(?P<CARDS>.*?)\](?:[ \t]+(?P<STRING>.+))?$".format(
            **substitutions,
        ),
        re.MULTILINE,
    )
    ...
    re_CollectPot2 = re.compile(
        r"^{PLYR} wins {CUR}(?P<POT>[{NUM}]+)".format(**substitutions),
        re.MULTILINE,
    )
    ...
    def readFile(self) -> bool | None:
        """Read the file, collapsing accidental double-spacing.

        Some exports double every newline (content lines separated by "\\n\\n",
        section breaks by "\\n\\n\\n\\n"). That both shatters a single hand across
        the "\\n\\n\\n+" splitter and breaks the multi-line header regexes, whose
        inter-line "\\s" cannot span a blank line. When clearly double-spaced,
        halve each newline run so the hand reads as normal single-spaced text.
        """
        ...
        if self.obs:
            ...
            if runs and even > odd and any(n >= 4 for n in runs):
                self.obs = re.sub(r"\n+", lambda m: "\n" * (len(m.group()) // 2 or 1), self.obs)
                self.whole_file = re.sub(
                    r"\n+", lambda m: "\n" * (len(m.group()) // 2 or 1), self.whole_file
                )
        return result
    ...
    def determineGameType(self, handText):
        ...
        self.site_id = self.sites[m.group("SITE")][1]  # Needs to match id entry in Sites database
            ...
            if key == "BUYIN" and hand.tourNo is not None:
                ...
                if self.sitename != "Merge" or hand.buyin == 0:
                    if info[key] == "Freeroll" or self.re_FreerollName.search(tourneyname):
                        ...
                        # Progressive-knockout buy-ins have a middle bounty part
                        # (e.g. "€22,25 + €22,25 + €5,50"): buy-in + bounty + fee.
                        if info.get("BIBOUNTY") is not None:
                            info["BIBOUNTY"] = self.clearMoneyString(info["BIBOUNTY"].strip("$€£"))
                            hand.koBounty = int(100 * Decimal(info["BIBOUNTY"]))
                            hand.isKO = True
    ...
    def readBlinds(self, hand) -> None:
        ...
        for a in self.re_PostBB.finditer(hand.handText):
            ...
            if not hand.gametype["bb"]:
                ...
                # A returning player posts "both" = big blind + dead small blind.
                # Use the actual small blind; the old (bb/2).to_integral_value()
                # rounded the SB to 0 at micro stakes, mistagging a plain second
                # big blind (a dead BB) as "both" and inflating the pot.
                sb_amt = Decimal(hand.gametype["sb"]) if hand.gametype.get("sb") else Decimal(hand.gametype["bb"]) / 2
                both = Decimal(hand.gametype["bb"]) + sb_amt
        ...
        for street, text in list(hand.streets.items()):
            ...
            for found in m:
                ...
                if found.group("NEWCARDS") is None:
                    ...
                    newcards = [
                        c.replace("10", "T").strip() for c in found.group("NEWCARDS").replace(" of ", "").split(", ")
                    ]
                else:
                    newcards = [c.replace("10", "T").strip() for c in found.group("NEWCARDS").split(" ")]
        ...
        for m in re_ShownCards.finditer(hand.handText):
            if m.group("CARDS") is not None and m.group("PNAME") not in found:
                ...
                if m.group("SHOWED") in ("shows", "Shows"):
                    shown = True
                elif m.group("SHOWED") in ("mucked", "mucks", "Mucks"):
                    mucked = True
                found.append(m.group("PNAME"))

                combo = m.groupdict().get("STRING")
                ...
                hand.addShownCards(
                    ...
                    shown=shown,
                    mucked=mucked,
                    string=combo.strip() if combo else None,
                )
