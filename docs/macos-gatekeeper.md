...
PyOxidizer is the only published macOS build. When the macOS release secrets
are configured, GitHub Release builds are signed with one stable Developer ID
Application identity, hardened, notarized, and stapled. The 3.7.0 workflow
still has an explicit ad-hoc fallback when `MACOS_SIGNING_IDENTITY` is absent;
such an artifact is suitable for testing one exact build but does not provide
stable privacy permissions or notarization.
...
application in `/Applications`. If the installed build is ad hoc, clear the
stale FPDB-only privacy records once:
...
missing permissions. Do not repeat this reset for normal upgrades of a
Developer ID-signed release: subsequent releases retain the same identity so
macOS can preserve those grants. Ad-hoc builds have a new identity on each
build and may require permission grants again.
...
Use a Developer ID-signed and notarized pre-release or release build for
acceptance testing of Screen Recording and Accessibility across upgrades. A
pull-request or fallback ad-hoc artifact can exercise the feature within one
run, but it cannot validate that permissions survive an upgrade.
