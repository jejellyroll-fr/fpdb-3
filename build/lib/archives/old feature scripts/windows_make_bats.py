...
try:
    ...
    dirs = [
        p
        for p in dirs
        if not re.search("python", p, re.I)
        or re.search("python25$", p, re.I)
        or re.search("python26$", p, re.I)
    ]
    # find gtk dirs:
    gtkdirs = [p for p in dirs if re.search("gtk", p, re.I)]

    lines = ["@echo off\n\n", "<path goes here>", "python fpdb.py\n\n", "pause\n\n"]
    if gtkdirs:
        i = 1
        ...
        print(
            ("No gtk directories found in your path.")
            + " "
            + ("Install gtk or edit the path manually.")
        )
