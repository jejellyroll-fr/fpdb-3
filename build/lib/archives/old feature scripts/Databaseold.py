...
class Database(object):
    ...
    def __init__(self, c, sql=None, autoconnect=True):
        ...
        if autoconnect:
            ...
            self.saveActions = (
                False if self.import_options["saveActions"] == False else True
            )
    ...
    def check_version(self, database, create):
        ...
        try:
            ...
            if settings[0] != DB_VERSION:
                log.error(
                    (("Outdated or too new database version (%s).") % (settings[0]))
                    + " "
                    + ("Please recreate tables.")
                )
        ...
        if self.backend != self.SQLITE:
            ...
            for i in range(maxtimes):
                try:
                    ...
                    log.debug(
                        ("commit %s failed: info=%s value=%s")
                        % (str(i), str(sys.exc_info()), str(sys.exc_info()[1]))
                    )
                    sleep(pause)
                if ok:
                    break
    ...
    def get_stats_from_hand(
        ...
        ptime = time() - stime
        log.info(
            "HudCache query get_stats_from_hand_aggregated took %.3f seconds" % ptime
        )
    ...
    def get_stats_from_hand_session(
        ...
        if colnames[0].lower() == "player_id":
            ...
            log.error(
                ("ERROR: query %s result does not have player_id as first column")
                % (query,)
            )
        ...
        try:
            if self.backend == self.MYSQL_INNODB:
                ...
                if ret < 1 or ret > 999999999:
                    log.warning(
                        ("getLastInsertId(): problem fetching insert_id? ret=%d") % ret
                    )
                    ret = -1
                ...
                if not row:
                    log.warning(
                        ("getLastInsertId(%s): problem fetching lastval? row=%d")
                        % (seq, row)
                    )
                    ret = -1
                else:
            ...
            log.error(
                ("*** Database get_last_insert_id error: ") + str(sys.exc_info()[1])
            )
            ...
            if fk["drop"] == 1:
                if self.backend == self.MYSQL_INNODB:
                    ...
                    try:
                        ...
                        c.execute(
                            "lock table %s in exclusive mode nowait" % (fk["fktab"],)
                        )
                        # print "after lock, status:", c.statusmessage
                        # print "alter table %s drop constraint %s_%s_fkey" % (fk['fktab'], fk['fktab'], fk['fkcol'])
                        try:
                            c.execute(
                                "alter table %s drop constraint %s_%s_fkey"
                                % (fk["fktab"], fk["fktab"], fk["fkcol"])
                            )
                            print(
                                "dropped pg fk pg fk %s_%s_fkey, continuing ..."
                                % (fk["fktab"], fk["fkcol"])
                            )
                        except:
                            if "does not exist" not in str(sys.exc_info()[1]):
                                print(
                                    (
                                        "warning: drop pg fk %s_%s_fkey failed: %s, continuing ..."
                                    )
                                    % (
                                        fk["fktab"],
                                        fk["fkcol"],
                                        str(sys.exc_info()[1]).rstrip("\n"),
                                    )
                                )
            ...
            if idx["drop"] == 1:
                if self.backend == self.MYSQL_INNODB:
                    ...
                    try:
                        ...
                        c.execute("BEGIN TRANSACTION")
                        c.execute(
                            "lock table %s in exclusive mode nowait" % (idx["tab"],)
                        )
                        # print "after lock, status:", c.statusmessage
                        try:
                            ...
                            if "does not exist" not in str(sys.exc_info()[1]):
                                print(
                                    (
                                        "warning: drop index %s_%s_idx failed: %s, continuing ..."
                                    )
                                    % (
                                        idx["tab"],
                                        idx["col"],
                        ...
                        print(
                            ("warning: index %s_%s_idx not dropped %s, continuing ...")
                            % (
                                idx["tab"],
                                idx["col"],
                                str(sys.exc_info()[1]).rstrip("\n"),
        ...
        for idx in self.indexes[self.backend]:
            if idx["drop"] == 1:
                if self.backend == self.MYSQL_INNODB:
                    ...
                    try:
                        s = "create index %s_%s_idx on %s(%s)" % (
                            idx["tab"],
                            idx["col"],
                            idx["tab"],
                            idx["col"],
    ...
    def dropAllIndexes(self):
        ...
        for idx in self.indexes[self.backend]:
            if self.backend == self.MYSQL_INNODB:
                ...
                try:
                    self.get_cursor().execute(
                        "drop index %s_%s_idx" % (idx["tab"], idx["col"])
                    )
                except:
                    print((("Drop index failed:"), str(sys.exc_info())))
            elif self.backend == self.SQLITE:
            ...
            if self.backend == self.MYSQL_INNODB:
                ...
                if cons:
                    ...
                    try:
                        ...
                        print(
                            ("Warning:"),
                            ("Drop foreign key %s_%s_fkey failed: %s, continuing ...")
                            % (
                                fk["fktab"],
                                fk["fkcol"],
                                str(sys.exc_info()[1]).rstrip("\n"),
                ...
                try:
                    ...
                    try:
                        ...
                        print(
                            ("dropped foreign key %s_%s_fkey, continuing ...")
                            % (fk["fktab"], fk["fkcol"])
                        )
                    except:
                        if "does not exist" not in str(sys.exc_info()[1]):
                    ...
                    print(
                        ("Warning:"),
                        ("constraint %s_%s_fkey not dropped: %s, continuing ...")
                        % (
                            fk["fktab"],
                            fk["fkcol"],
                            str(sys.exc_info()[1]).rstrip("\n"),
    ...
    def rebuild_cache(
        ...
        if not ttid:
            if self.hero_ids is None:
                if wmid:
                    where = (
                        "WHERE g.type = 'ring' AND weekId = %s and monthId = %s<hero_where>"
                        % wmid
                    )
                else:
        ...
        if ttid:
            ...
            if wmid:
                where = (
                    "WHERE g.type = 'tour' AND weekId = %s and monthId = %s<hero_where>"
                    % wmid
                )
            else:
    ...
    def get_hero_hudcache_start(self):
        ...
        try:
            ...
            err = traceback.extract_tb(sys.exc_info()[2])[-1]
            print(("Error rebuilding hudcache:"), str(sys.exc_info()[1]))
            print(err)
    ...
    def releaseLock(self):
        if self._has_lock:
            cursor = self.get_cursor()
            num = cursor.execute(
                self.sql.query["switchLockOff"], (False, self.threadId)
            )
            self.commit()
            self._has_lock = False
        ...
        if doinsert:
            ...
            for i in range(len(self.s["bk"])):
                ...
                if self.s["bk"][i]["tourneys"]:
                    toursql = (
                        "OR SC.id in (SELECT DISTINCT sessionId FROM Tourneys T WHERE T.id in (%s))"
    ...
    def storeCardsCache(
        ...
        if doinsert:
            ...
            update_cardscache = update_cardscache.replace(
                "%s", self.sql.query["placeholder"]
            )
            insert_cardscache = self.sql.query["insert_cardscache"]
            insert_cardscache = insert_cardscache.replace(
                "%s", self.sql.query["placeholder"]
            )
            select_cardscache_ring = self.sql.query["select_cardscache_ring"]
            select_cardscache_ring = select_cardscache_ring.replace(
                "%s", self.sql.query["placeholder"]
            )
            select_cardscache_tour = self.sql.query["select_cardscache_tour"]
            select_cardscache_tour = select_cardscache_tour.replace(
                "%s", self.sql.query["placeholder"]
            )
    ...
    def storePositionsCache(
        ...
        if doinsert:
            ...
            update_positionscache = update_positionscache.replace(
                "%s", self.sql.query["placeholder"]
            )
            insert_positionscache = self.sql.query["insert_positionscache"]
            insert_positionscache = insert_positionscache.replace(
                "%s", self.sql.query["placeholder"]
            )

            select_positionscache_ring = self.sql.query["select_positionscache_ring"]
            select_positionscache_ring = select_positionscache_ring.replace(
                "%s", self.sql.query["placeholder"]
            )
            select_positionscache_tour = self.sql.query["select_positionscache_tour"]
            select_positionscache_tour = select_positionscache_tour.replace(
                "%s", self.sql.query["placeholder"]
            )
    ...
    def getHeroIds(self, pids, sitename):
        ...
        try:
            ...
            err = traceback.extract_tb(sys.exc_info()[2])[-1]
            # print ("Error aquiring hero ids:"), str(sys.exc_value)
        return hero_ids
    ...
    def getSqlTourneyTypeIDs(self, hand):
        ...
        result = self.createOrUpdateTourneyType(
            hand
            ...
            for entryIdx in range(len(entries)):
                ...
                if (playerId, entryId) in tplayers:
                    ...
                    for ev in expectedValues:
                        ...
                        if (
                            summaryDict[player][entryIdx] == None
                            and resultDict[ev[1]] != None
                        ):  # DB has this value but object doesnt, so update object
