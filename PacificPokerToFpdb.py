...
class PacificPoker(HandHistoryConverter):
    ...
    re_Identify     = re.compile(u'\*{4,5}\s(Cassava|888poker|888)(\.[a-z]{2})?(\-[a-z]{2})?\s(Snap\sPoker\s|BLAST\s)?Hand\sHistory\sfor\sGame\s\d+\s')
            ...
            if key == 'BUYIN' and info['BUYIN'] != None:
                if info[key] == 'Free' or info['BIAMT'] is None:
                    ...
                    if info['BUYIN'].find("$")!=-1:
                        ...
                        log.error(("PacificPokerToFpdb.readHandInfo: Failed to detect currency.") + " Hand ID: %s: '%s'" % (hand.handid, info[key]))
                        raise FpdbParseError
