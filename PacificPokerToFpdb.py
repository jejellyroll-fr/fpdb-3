...
class PacificPoker(HandHistoryConverter):
    ...
    re_PostBB = re.compile(
        r"^%(PLYR)s posts big blind \[(%(CUR)s)?\s?(?P<BB>[%(NUM)s]+)\s?(%(CUR)s)?\]"
        % substitutions,
        re.MULTILINE,
    )
    re_Antes = re.compile(
        r"^%(PLYR)s posts (the\s)?ante \[(%(CUR)s)?\s?(?P<ANTE>[%(NUM)s]+)\s?(%(CUR)s)?\]"
        % substitutions,
        re.MULTILINE,
    )
    # TODO: unknown in available hand histories for pacificpoker:
    re_BringIn = re.compile(
        r"^%(PLYR)s: brings[- ]in( low|) for (%(CUR)s)?\s?(?P<BRINGIN>[%(NUM)s]+)\s?(%(CUR)s)?"
        % substitutions,
        re.MULTILINE,
    )
    ...
    re_CollectPot = re.compile(
        r"^%(PLYR)s collected \[ (%(CUR)s)?\s?(?P<POT>[%(NUM)s]+)\s?(%(CUR)s)? \]$"
        % substitutions,
        re.MULTILINE,
    )
