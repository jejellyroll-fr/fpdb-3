...
def testFlopImport():
    ...
    print(
        "DEBUG: stored: %s dups: %s partial: %s errs: %s ttime: %s"
...
def testStudImport():
    ...
    importer.setThreads(-1)
    importer.addBulkImportImportFileOrDir(
        """regression-test-files/cash/Stars/Stud/7-Stud-USD-0.04-0.08-200911.txt""",
        site="PokerStars",
    )
    importer.addBulkImportImportFileOrDir(
        """regression-test-files/cash/Stars/Stud/7-StudHL-USD-0.04-0.08-200911.txt""",
        site="PokerStars",
    )
    importer.addBulkImportImportFileOrDir(
        """regression-test-files/cash/Stars/Stud/Razz-USD-0.04-0.08-200911.txt""",
        site="PokerStars",
    )
    importer.setCallHud(False)
    (stored, dups, partial, errs, ttime) = importer.runImport()
...
def testDrawImport():
    try:
        ...
        importer.setThreads(-1)
        importer.addBulkImportImportFileOrDir(
            """regression-test-files/cash/Stars/Draw/3-Draw-Limit-USD-0.10-0.20-200911.txt""",
            site="PokerStars",
        )
        importer.addBulkImportImportFileOrDir(
            """regression-test-files/cash/Stars/Draw/5-Carddraw-USD-0.10-0.20-200911.txt""",
            site="PokerStars",
        )
        importer.setCallHud(False)
        (stored, dups, partial, errs, ttime) = importer.runImport()
