...
def remove_tree(top):
    ...
    #     make it a bit safer
    if (top in ("build", "dist") or top.startswith(distdir)) and os.path.basename(
        os.getcwd()
    ) == "windows":
...
setup(
    ...
    windows=[
        {
            "script": pydir + "fpdb.pyw",
            "uac_info": "requireAdministrator",
            "icon_resources": [(1, gfxdir + "fpdb_large_icon.ico")],
        },
        {
    ...
    options={
        "py2exe": {
            ...
            "dll_excludes": [
                ...
                "MSVCR90.dll",
                "msvcr90.dll",
            ],  # these are vis c / c++ runtimes, and must not be redistributed
        }
    },
...
copy_tree(
    r"c:\python27\Lib\site-packages\pytz\zoneinfo", os.path.join(r"dist", "zoneinfo")
)
...
while not os.path.exists(gtk_dir):
    print(
        "Enter directory name for GTK (e.g. c:/gtk) : ", end=" "
