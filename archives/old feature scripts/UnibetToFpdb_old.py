...
class Unibet(HandHistoryConverter):
    ...
    re_Identify = re.compile("^Game\s\#\d+:\sTable")
        ...
        if not players <= self.compiledPlayers:  # x <= y means 'x is subset of y'
            ...
            if self.siteId == 26:
                self.re_HeroCards = re.compile(
                    r"^Dealt to (?P<PNAME>(?![A-Z][a-z]+\s[A-Z]).+?)(?: \[(?P<OLDCARDS>.+?)\])?( \[(?P<NEWCARDS>.+?)\])"
                    % subst,
                    re.MULTILINE,
                )
        ...
        if info["limitType"] == "fl" and info["bb"] is not None:
            if info["type"] == "ring":
                try:
                    ...
                    log.error(
                        (
                            "UnibetToFpdb.determineGameType: Lim_Blinds has no lookup for '%s' - '%s'"
                        )
                        % (mg["BB"], tmp)
                    )
                    raise FpdbParseError
    ...
    def readHandInfo(self, hand):
        ...
        if hand.handText.count("*** SUMMARY ***") != 1:
            raise FpdbHandPartial(
                _("Hand is not cleanly split into pre and post Summary")
            )
                ...
                if hand.tourNo is not None:
                    ...
                    if info[key].strip() == "Freeroll":
                        ...
                        if info[key].find("$") != -1:
                            ...
                            log.error(
                                _(
                                    "UnibetToFpdb.readHandInfo: Failed to detect currency."
                                )
        ...
        for action in m:
            ...
            if action.group("ATYPE") == " folds":
                ...
                log.debug(
                    ...
                    % ("readAction", action.group("PNAME"), action.group("ATYPE"))
                )
