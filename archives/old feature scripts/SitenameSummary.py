...
class Sitename(TourneySummary):
    ...
    re_Player = re.compile(
        """(?P<RANK>[0-9]+):\s(?P<NAME>.*)\s\(.*\),(\s)?(\$(?P<WINNINGS>[0-9]+\.[0-9]+))?(?P<STILLPLAYING>still\splaying)?((?P<TICKET>Tournament\sTicket)\s\(WSOP\sStep\s(?P<LEVEL>\d)\))?(\s+)?"""
    )
    ...
    def parseSummary(self):
        ...
        if m == None:
            ...
            log.error(
                "parseSummary: " + ("Unable to recognise Tourney Info: '%s'") % tmp
            )
            log.error("parseSummary: " + ("Raising FpdbParseError"))
            raise FpdbParseError(("Unable to recognise Tourney Info: '%s'") % tmp)
