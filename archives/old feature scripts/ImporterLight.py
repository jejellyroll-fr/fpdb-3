...
class Importer(object):
        ...
        if site not in self.siteIds:
            ...
            if len(result) == 1:
                ...
                if len(result) == 0:
                    ...
                    log.error(
                        ("More than 1 Database ID found for %s") % fpdbfile.site.name
                    )
    ...
    def addImportDirectory(
        ...
        if os.path.isdir(dir):
            ...
            logging.warning(
                f"Attempted to add non-directory '{dir}' as an import directory"
            )
                ...
                try:
                    if moveimportedfiles:
                        imported_path = os.path.join(
                            os.getcwd(),
                            "fpdbimported",
                            f"{filecount}-{os.path.basename(f)[3:]}",
                        )
                    ...
                    if movefailedfiles:
                        failed_path = os.path.join(
                            os.getcwd(),
                            "fpdbfailed",
                            f"{fileerrorcount}-{os.path.basename(f)[3:]}",
                        )
            ...
            if stored > 0:
                ...
                for i in range(len(phands)):
                    ...
                    try:
                        ...
                        log.error(
                            ("Importer._import_hh_file: '%r' Fatal error: '%r'")
                            % (fpdbfile.path, error_trace)
                        )
                        log.error(("'%r'") % tmp)
                        if doinsert and ihands:
                            backtrack = True
                ...
                if self.callHud:
                    ...
                    for hid in list(to_hud):
                        try:
                            ...
                            print(
                                "self.caller.pipe_to_hud.stdin.write",
                                self.caller.pipe_to_hud.stdin.write,
                            )
                            print(
                                ("fpdb_import: sending hand to hud"),
                                hid,
                                "pipe =",
                                self.caller.pipe_to_hud,
                            )
                            self.caller.pipe_to_hud.stdin.write(
                                "%s" % (hid) + os.linesep
                            )
        ...
        if callable(obj):
            ...
            for j, summaryText in enumerate(summaryTexts, start=1):
                ...
                try:
                    ...
                    log.error(
                        ("Summary import parse error in file: %s") % fpdbfile.path
                    )
                    errors += 1
                if j != 1:
                    print(
