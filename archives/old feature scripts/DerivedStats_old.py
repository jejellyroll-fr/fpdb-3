...
class DerivedStats(object):
    ...
    def assembleHandsPlayers(self, hand):
        ...
        for player, money_committed in list(hand.pot.committed.items()):
            ...
            committed_player_stats["totalProfit"] = int(
                committed_player_stats["winnings"] - paid
            )
        ...
        for i, player in enumerate(contributed):
            self.handsplayers[player]["rakeContributed"] = (
                100 * hand.rake / len(contributed)
            )
        ...
        for i, street in enumerate(hand.actionStreets):
            for j, act in enumerate(hand.actions[street]):
                ...
                if act[1] in ("discards"):
                    ...
                    self.handsplayers[act[0]]["street%dDiscards" % (i - 1)] = int(
                        act[2]
                    )
                ...
                for street, board in list(boards.items()):
                    ...
                    if (
                        ...
                        for n in range(len(board["board"])):
                            ...
                            cards += (
                                board["board"][n]
                                if (board["board"][n] and "omaha" not in evalgame)
                                else []
                            )
                            bcards = (
                                board["board"][n]
                                if (board["board"][n] and "omaha" in evalgame)
                                else []
                            )
                            cards = [
                            ...
                            bcards = [
                                str(b) if Card.encodeCardList.get(b) else "0x"
                                for b in bcards
                            ]
                ...
                for n in range(len(board["board"])):
                    ...
                    if len(players) == len(valid) and (board["allin"] or hand.publicDB):
                        ...
                        if len(valid) > 1:
                            ...
                            equities = [1000]
                        remainder = old_div(
                            (1000 - sum(equities)), Decimal(len(equities))
                        )
                        for i in range(len(equities)):
                            ...
                            if street == startstreet:
                                rake = hand.rake * (
                                    old_div(Decimal(pot), Decimal(hand.totalpot))
                                )
                                holecards[p]["eq"] += old_div(
                                    ((pot - rake) * equities[i]), Decimal(10)
                                )
    ...
    def getBoardsDict(self, hand, base, streets):
        ...
        if base == "hold":
            for s in allInStreets:
                ...
                b = [
                    x
                    for sublist in [hand.board[k] for k in allInStreets[: streetId + 1]]
                    for x in sublist
                ]
    ...
    def awardPots(self, hand):
        ...
        if pokereval and len(hand.pot.pots) > 1 and evalgame and holeshow:
            ...
            board = [
                str(b)
                for b in hand.board["FLOP"] + hand.board["TURN"] + hand.board["RIVER"]
            ]
            for pot, players in hand.pot.pots:
                if potId == 0:
                    pot += sum(hand.pot.common.values()) + hand.pot.stp
                ...
                for p in players:
                    ...
                    if "omaha" not in evalgame:
                        holes = [
                            str(c) for c in hcs[hrange[-1][0] : hrange[-1][1]] + board
                        ]
                        bcards = []
                    else:
                ...
                if not totalrake:
                    ...
                    if totalpot:
                        ...
                        totalrake = 0
                rake = totalrake * (old_div(pot, hand.totalpot))
                for w in win["hi"]:
                    pname = list(players)[w]
                    ppot = str(
                        (old_div((pot - rake), len(win["hi"]))).quantize(
    ...
    def assembleHandsPots(self, hand):
        category, positions, playersPots, potFound, positionDict, showdown = (
            hand.gametype["category"],
            [],
            {},
            {},
        ...
        if evalgame and (
            ...
            for pot, players in hand.pot.pots:
                ...
                for b in boards:
                    ...
                    potBoard = old_div(Decimal(int(pot / len(boards) * factor)), factor)
                ...
                if hand.collectees.get(p) and info:
                    ...
                    for item in info:
                        ...
                        split = [
                            ...
                            if len(playersPots[n][1]) == 1
                            and hand.collectees.get(n) != None
                        ]
                        if len(info) == 1:
                            ppot = item["ppot"]
                            ...
                            rake = (totalrake * (old_div(ppot, total))).quantize(
                                Decimal("0.01")
                            )
                            collected = ppot - rake
                        potFound[p][0] -= ppot
