...
if not options.errorsToConsole:
    print(
        (("Note: error output is being diverted to %s.") % "tourneyerror.txt"), end=" "
    )
    print(("Any major error will be reported there _only_."))
...
class Tournament(object):
    ...
    def __init__(
        self, parent, site, tid
    ):  # site should probably be something in the config object, but i don't know how the config object works right now, so we're going to make it a str ..
        ...
        self.players = (
            {}
        )  # eventually i'd guess we'd probably want to fill this with playername:playerid's
        self.results = (
            {}
        )  # i'd guess we'd want to load this up with playerid's instead of playernames, too, as well, also
        ...
        self.buyin = (
            summary.parser.TourneyBuyIn
        )  # need to remember to parse the Fee out of this and move it to self.fee
...
class ttracker_main(object):
    ...
    def __init__(self, db_name="fpdb"):
        ...
        self.addbutton = gtk.Button(label=("Add Tournament"))
    ...
    def addClicked(
        self, widget, data
    ):  # what is "data"? i'm guessing anything i pass in after the function name in connect() but unsure because the documentation sucks
        ...
        if t is not None:
            ...
            self.vb.add(
                editbutton
            )  # These should probably be put in.. a.. h-box? i don't know..
            self.vb.add(mylabel)
    ...
    def create_HUD(
        self, new_hand_id, table, table_name, max, poker_game, stat_dict, cards
    ):
        ...
        def idle_func():
            ...
            try:
                ...
                self.hud_dict[table_name].create(
                    new_hand_id, self.config, stat_dict, cards
                )
        ...
        self.hud_dict[table_name] = Hud.Hud(
            self, table, max, poker_game, self.config, self.db_connection
        )
    ...
    def read_stdin(self):  # This is the thread function
        ...
        while 1:  # wait for a new hand number on stdin
            ...
            try:
                (table_name, max, poker_game, type, fast, site_id, numseats) = (
                    self.db_connection.get_table_name(new_hand_id)
                )
                ...
                if (
                    new_hand_id
                ):  # new_hand_id is none if we had an error prior to the store
                    sys.stderr.write(
            ...
            if type == "tour":  # hand is from a tournament
                ...
                if mat_obj:
                    ...
                    sys.stderr.write(
                        ("Could not find tournament %d in hand %d. Skipping.")
            ...
            #    Or create a new HUD
                ...
                if tablewindow is None:
                    ...
                    sys.stderr.write(
                        ("Table name %s not found, skipping.") % table_name
                    )
                else:
                    self.create_HUD(
                        new_hand_id,
...
if __name__ == "__main__":
    ...
    sys.stderr.write(("Using db name = %s") % (options.dbname))
