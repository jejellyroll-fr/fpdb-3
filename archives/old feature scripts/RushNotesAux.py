...
from Mucked import Aux_Seats, Aux_Window, Seat_Window
...
minidom.Element.writexml = instancemethod(writexml, None, minidom.Element)
...
class RushNotes(Aux_Window):
    ...
    def __init__(self, hud, config, params):
        ...
        notepath = site_params_dict[
            "site_path"
        ]  # this is a temporary hijack of site-path
        self.heroid = self.hud.db_connection.get_player_id(
            self.config, sitename, heroname
        )
    ...
    def update_data(self, new_hand_id, db_connection):
        ...
        for playerid in handplayers:
            ...
            if playerid == self.heroid:
                continue

            playername = str(
                str(
                    Stats.do_stat(
                        self.hud.stat_dict, player=playerid, stat="playername"
                    )[1]
                )
            )
            # Use index[3] which is a short description
            n = str(
                Stats.do_stat(self.hud.stat_dict, player=playerid, stat="n")[3] + " "
            )
            vpip = str(
                Stats.do_stat(self.hud.stat_dict, player=playerid, stat="vpip")[3] + " "
            )
            pfr = str(
                Stats.do_stat(self.hud.stat_dict, player=playerid, stat="pfr")[3] + " "
            )
            three_B = str(
                Stats.do_stat(self.hud.stat_dict, player=playerid, stat="three_B")[3]
                + " "
            )
            four_B = str(
                Stats.do_stat(self.hud.stat_dict, player=playerid, stat="four_B")[3]
                + " "
            )
            cbet = str(
                Stats.do_stat(self.hud.stat_dict, player=playerid, stat="cbet")[3] + " "
            )

            fbbsteal = str(
                Stats.do_stat(self.hud.stat_dict, player=playerid, stat="f_BB_steal")[3]
                + " "
            )
            f_3bet = str(
                Stats.do_stat(self.hud.stat_dict, player=playerid, stat="f_3bet")[3]
                + " "
            )
            f_4bet = str(
                Stats.do_stat(self.hud.stat_dict, player=playerid, stat="f_4bet")[3]
                + " "
            )

            steal = str(
                Stats.do_stat(self.hud.stat_dict, player=playerid, stat="steal")[3]
                + " "
            )
            ffreq1 = str(
                Stats.do_stat(self.hud.stat_dict, player=playerid, stat="ffreq1")[3]
                + " "
            )
            agg_freq = str(
                Stats.do_stat(self.hud.stat_dict, player=playerid, stat="agg_freq")[3]
                + " "
            )
            BBper100 = str(
                Stats.do_stat(self.hud.stat_dict, player=playerid, stat="BBper100")[3]
            )
            if BBper100[6] == "-":
                BBper100 = BBper100[0:6] + "(" + BBper100[7:] + ")"
            ...
            c.execute(
                (
                    "SELECT handId, position, startCards, street0Aggr, tableName "
            ...
            for qid, qposition, qstartcards, qstreet0Aggr, qtablename in c.fetchall():
                if debugmode:
                    debugfile.write(
                        "pid, hid, pos, cards, aggr, table player"
                        + str(playerid)
                        + "/"
                        + str(qid)
                        + "/"
        ...
        if debugmode:
            now = datetime.now()
            debugfile.write(
                now.strftime("%Y%m%d%H%M%S") + " villain data has been processed" + "\n"
            )
        ...
        for noteentry in notelist:  # for each note in turn
            noteplayer = noteentry.getAttribute(
                "PlayerId"
            )  # extract the playername from xml
        ...
        if debugmode:
            now = datetime.now()
            debugfile.write(
                now.strftime("%Y%m%d%H%M%S") + " xml pre-processing complete" + "\n"
            )
        ...
        if debugmode:
            now = datetime.now()
            debugfile.write(
                now.strftime("%Y%m%d%H%M%S") + " dom written, process finished" + "\n"
            )
            debugfile.close()
