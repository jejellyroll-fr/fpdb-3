...
class GuiDatabase(object):
    ...
    def edited_cb(self, cell, path, new_text, user_data):
        ...
        if valid:
            ...
            self.config.set_db_parameters(
                db_server=self.liststore[path][self.COL_DBMS],
                db_name=name,
        ...
        if event.button == 1:  # and event.type == gtk.gdk._2BUTTON_PRESS:
            ...
            if pthinfo is not None:
                ...
                if col == self.listcols[self.COL_DFLT]:
                    if (
                        self.liststore[row][self.MODEL_STATUS] == "ok"
                        and self.liststore[row][self.MODEL_DFLTIC] is None
                    ):
    ...
    def setDefaultDB(self, row):
        ...
        for r in range(len(self.liststore)):
            ...
            self.config.set_db_parameters(
                db_server=self.liststore[r][self.COL_DBMS],
    ...
    def loadDbs(self):
        ...
        try:
            ...
            for (
                name
            ) in self.config.supported_databases:  # db_ip/db_user/db_pass/db_server
                dbms = self.config.supported_databases[
                    name
                ].db_server  # mysql/postgresql/sqlite
                ...
                default_icon = None
                if default:
                    default_icon = gtk.STOCK_APPLY

                status, err_msg, icon = GuiDatabase.testDB(
                    self.config, dbms, dbms_num, name, user, passwd, host
                )
    ...
    def sortCols(self, col, n):
        try:
            ...
            if (
                not col.get_sort_indicator()
                or col.get_sort_order() == gtk.SORT_ASCENDING
            ):
    ...
    def addDB(self, widget, data):
        ...
        (status, err_msg, icon, dbms, dbms_num, name, comment, user, passwd, host) = (
            adb.run()
        )
        adb.destroy()
        ...
        if status == "ok":
            iter = self.liststore.append(
                (dbms, name, comment, user, passwd, host, "", None, status, icon)
            )
    ...
    def testDB(config, dbms, dbms_num, name, user, passwd, host):
        ...
        try:
            ...
            log.debug(
                ...
                + " %s/%s, %s, %s/%s" % (str(dbms_num), dbms, name, user, passwd)
            )
            db.connect(
                backend=dbms_num,
                host=host,
            ...
            err_msg = (
                "MySQL client reports: 2002 or 2003 error. Unable to connect - "
        ...
        if err_msg:
            log.info(
                ("db connection to %s, %s, %s, %s, %s failed: %s")
                % (str(dbms_num), host, name, user, passwd, err_msg)
            )
...
class AddDB(gtk.Dialog):
    ...
    def run(self):
        ...
        log.debug(
            ("addDB.run: response is %s, accept is %s")
            % (str(response), str(int(gtk.RESPONSE_ACCEPT)))
        )
        ...
        (status, err_msg, icon, dbms, dbms_num, name, db_desc, user, passwd, host) = (
            "error",
            "error",
            None,
            None,
    ...
    def check_fields(self):
        ...
        if self.e_db_name.get_text() == "":
            ...
            self.cb_dbms.get_active_text() != "Sqlite"
            and self.e_username.get_text() == ""
        ):
            msg = "No Username given"
            ok = False
        elif (
            self.cb_dbms.get_active_text() != "Sqlite"
            and self.e_password.get_text() == ""
        ):
...
class InfoBox(gtk.Dialog):
    ...
    def __init__(self, parent, str1):
        ...
        super(InfoBox, self).__init__(
            parent=parent, flags=gtk.DIALOG_DESTROY_WITH_PARENT, title=""
        )  # , buttons=btns
