        ...
        if info['limitType'] == 'fl' and info['bb'] is not None:
            if info['type'] == 'ring':
                try:
                    ...
                    tmp = handText[0:200]
                    log.error(("PokerTrackerToFpdb.determineGameType: Lim_Blinds has no lookup for '%s' - '%s'") % (mg['BB'], tmp))
                    raise FpdbParseError
            else:
                    ...
                    if self.sitename != 'Merge' or hand.buyin==0:
                        if info[key] == 'Freeroll' or 'Free' in tourneyname:
                            ...
                            if info[key].find("$")!=-1:
                                ...
                                log.error(("PokerTrackerToFpdb.readHandInfo: Failed to detect currency.") + " Hand ID: %s: '%s'" % (hand.handid, info[key]))
                                raise FpdbParseError
