...
class PokerTracker(HandHistoryConverter):
    ...
    re_Site = re.compile(
        "(?P<SITE>EverestPoker\sGame\s\#|GAME\s\#|MERGE_GAME\s\#|Merge\sGame\s\#|\*{2}\s(Game\sID|Hand\s\#)\s)\d+"
    )
    # Static regexes
    ...
    re_HandInfo_Cash = re.compile(
        """
          ^Table\s(?P<TABLE>[^,]+?)(,\sSeats\s(?P<MAX>\d+))?$"""
        % substitutions,
        re.MULTILINE | re.VERBOSE,
    )

    re_Identify = re.compile(
        "(EverestPoker\sGame\s\#|GAME\s\#|MERGE_GAME\s\#|Merge\sGame\s\#|\*{2}\s(Game\sID|Hand\s\#)\s)\d+"
    )
    ...
    re_DateTime2 = re.compile(
        """(?P<M>[0-9]{2})\/(?P<D>[0-9]{2})\/(?P<Y>[0-9]{4})[\- ]+(?P<H>[0-9]+):(?P<MIN>[0-9]+):(?P<S>[0-9]+)""",
        re.MULTILINE,
    )
    re_DateTime3 = re.compile(
        """(?P<H>[0-9]+):(?P<MIN>[0-9]+):(?P<S>[0-9]+)[\- ]+(?P<Y>[0-9]{4})\/(?P<M>[0-9]{2})\/(?P<D>[0-9]{2})""",
        re.MULTILINE,
    )
    ...
    re_PostSB = re.compile(
        r"^%(PLYR)s:? ((posts|posted) the small blind( of)?|(Post )?SB) (\- )?%(CUR)s(?P<SB>[%(NUM)s]+)"
        % substitutions,
        re.MULTILINE,
    )
    re_PostBB = re.compile(
        r"^%(PLYR)s:? ((posts|posted) the big blind( of)?|posts the dead blind of|(Post )?BB) (\- )?%(CUR)s(?P<BB>[%(NUM)s]+)"
        % substitutions,
        re.MULTILINE,
    )
    re_Antes = re.compile(
        r"^%(PLYR)s:? ((posts|posted) (the )?ante( of)?|(Post )?Ante) (\- )?%(CUR)s(?P<ANTE>[%(NUM)s]+)"
        % substitutions,
        re.MULTILINE,
    )
