...
- ✅ **Dette source traçable** (2026-07-16) : les 73 `TODO/FIXME/HACK` initiaux sont inventoriés dans `TECHNICAL_DEBT.md`, avec identifiants stables, catégories et liens source ; la CI interdit que le registre diverge du code. La sélection CLI distingue désormais l'absence de `--databaseName` d'un nom explicitement vide ; `IdentifySite.Site` expose aussi les noms canoniques `hhc_fname`/`hhc_type` tout en conservant l'alias legacy `filter_name`. Le registre est ramené à 70 entrées.
