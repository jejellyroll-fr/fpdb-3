...
class Fulltilt(HandHistoryConverter):
    ...
    def determineGameType(self, handText):
        ...
        if not m:
            tmp = handText[0:200]
            log.error(("FulltiltToFpdb.determineGameType: '%s'") % tmp)
        ...
        if info['limitType'] == 'fl' and info['bb'] is not None:
            if info['type'] == 'ring':
                try:
                    ...
                    tmp = handText[0:200]
                    log.error(("FulltiltToFpdb.determineGameType: Lim_Blinds has no lookup for '%s' - '%s'") % (bb, tmp))
                    raise FpdbParseError
            else:
    ...
    def readHandInfo(self, hand):
        ...
        if m is None:
            ...
            log.error(("FulltiltToFpdb.readHandInfo: '%s'") % tmp)
            raise FpdbParseError
        ...
        if self.re_Cancelled.search(hand.handText):
            raise FpdbHandPartial(("Hand '%s' was cancelled.") % m.group('HID'))
    ...
    def readPlayerStacks(self, hand):
        ...
        if len(handsplit)!=2:
            raise FpdbHandPartial(("Hand is not cleanly split into pre and post Summary %s.") % hand.handid)
        pre, post = handsplit
        ...
        if plist == {}:
            ...
            raise FpdbHandPartial(("No players detected in hand %s.") % hand.handid)
    ...
    def readBringIn(self, hand):
        ...
        #else:
            #log.debug(("No bringin found, handid =%s") % hand.handid)
    ...
    def readButton(self, hand):
        try:
            ...
            raise FpdbHandPartial(("%s Failed to detect button (hand #%s cancelled?)") % ("readButton:", hand.handid))
