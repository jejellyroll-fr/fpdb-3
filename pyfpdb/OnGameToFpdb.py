...
#    They still identify the hero.
    ...
    def determineGameType(self, handText):
        ...
        if not m:
            tmp = handText[0:200]
            log.error(("OnGameToFpdb.determineGameType: '%s'") % tmp)
        ...
        if 'LIMIT' in mg:
            if mg['LIMIT'] in self.limits:
                ...
                tmp = handText[0:200]
                log.error(("OnGameToFpdb.determineGameType: Limit not found in '%s'") % tmp)
                raise FpdbParseError
        if 'GAME' in mg:
        ...
        if info['limitType'] == 'fl' and info['bb'] is not None:
            if info['type'] == 'ring':
                try:
                    ...
                    tmp = handText[0:200]
                    log.error(("OnGameToFpdb.determineGameType: Lim_Blinds has no lookup for '%s' - '%s'") % (bb, tmp))
                    raise FpdbParseError
            else:
    ...
    def readHandInfo(self, hand):
        ...
        if m is None:
            ...
            log.error(("OnGameToFpdb.readHandInfo: '%s'") % tmp)
            raise FpdbParseError
        ...
        for key in info:
            if key == 'DATETIME':
                ...
                if a:
                    ...
                    log.error("OnGameToFpdb.readHandInfo: " + ("DATETIME not matched: '%s'") % info[key])
                    raise FpdbParseError
...
#            hand.setCommunityCards(street, m.group('CARDS').split(','))
    ...
    def readBlinds(self, hand):
        try:
            ...
            log.debug(("No small blinds found.")+str(sys.exc_info()) )
