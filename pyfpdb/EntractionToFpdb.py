...
class Entraction(HandHistoryConverter):
    ...
    def determineGameType(self, handText):
        ...
        if not m:
            tmp = handText[0:200]
            log.error(("EntractionToFpdb.determineGameType: '%s'") % tmp)
        ...
        if info['limitType'] == 'fl' and info['bb'] is not None:
            if info['type'] == 'ring':
                try:
                    ...
                    tmp = handText[0:200]
                    log.error(("PokerStarsToFpdb.determineGameType: Lim_Blinds has no lookup for '%s' - '%s'") % (info['bb'], tmp))
                    raise FpdbParseError
            else:
...
#
    ...
    def readHandInfo(self, hand):
        ...
        if m is None or m2 is None:
            ...
            log.error(("EntractionToFpdb.readHandInfo: '%s'") % tmp)
            raise FpdbParseError
                ...
                if hand.tourNo!=None:
                    if info[key] == 'Freeroll' or info['BIAMT'] is None:
                        ...
                        if info[key].find("$")!=-1:
                            ...
                            log.error(("EntractionToFpdb.readHandInfo: Failed to detect currency.") + " Hand ID: %s: '%s'" % (hand.handid, info[key]))
                            raise FpdbParseError
