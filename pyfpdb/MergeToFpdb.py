            ...
            if fulltable != True:
                ...
                for seatno in list(acted.keys()):
                    if seatno not in seated:
                        log.error(("MergeToFpdb.readPlayerStacks: '%s' Seat:%s acts but not listed") % (hand.handid, seatno))
                        raise FpdbParseError
        ...
        for action in m:
            ...
            if player in hand.stacks and player not in hand.folded:
                if action.group('ATYPE') in ('FOLD', 'SIT_OUT'):
                    ...
                    log.debug(("Unimplemented %s: '%s' '%s'") % ("readAction", action.group('PSEAT'), action.group('ATYPE')))
