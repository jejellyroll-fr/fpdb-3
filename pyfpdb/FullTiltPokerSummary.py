...
class FullTiltPokerSummary(TourneySummary):
    ...
    def parseSummaryXLS(self):
        ...
        if m==None:
            tmp1 = info['header']
            tmp2 = str(info)[0:200]
            log.error(("FullTiltPokerSummary.parseSummaryXLS: '%s' '%s") % (tmp1, tmp2))
        ...
        if 'game type desc' in info:
            ...
            if m3:
                ...
                log.error(("FullTiltPokerSummary.parseSummaryXLS Game '%s' not found") % info['game type desc'])
                raise FpdbParseError
    ...
    def parseSummaryFile(self):
        ...
        if m == None:
            ...
            log.error(("FullTiltPokerSummary.parseSummary: '%s'") % tmp)
            raise FpdbParseError
        ...
        if m == None:
            log.error("FullTiltPokerSummary.parseSummary: " + ("Unable to locate currency"))
            raise FpdbParseError
        #print "DEBUG: m.groupdict(): %s" % m.groupdict()
