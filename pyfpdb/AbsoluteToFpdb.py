...
class Absolute(HandHistoryConverter):
    ...
    def determineGameType(self, handText):
        ...
        if not m:
            tmp = handText[0:200]
            log.error(("AbsoluteToFpdb.determineGameType: '%s'") % tmp)
        ...
        if info['limitType'] == 'fl' and info['bb'] is not None:
            if info['type'] == 'ring':
                try:
                    ...
                    tmp = handText[0:200]
                    log.error(("AbsoluteToFpdb.determineGameType: Lim_Blinds has no lookup for '%s' - '%s'") % (info['bb'], tmp))
                    raise FpdbParseError
        return info
    ...
    def readHandInfo(self, hand):
        ...
        if m is None:
            ...
            log.error(("AbsoluteToFpdb.readHandInfo: '%s'") % tmp)
            raise FpdbParseError
