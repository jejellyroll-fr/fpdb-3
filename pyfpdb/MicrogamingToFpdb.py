...
class Microgaming(HandHistoryConverter):
    ...
    def determineGameType(self, handText):
        ...
        if not m: 
            tmp = handText[0:200]
            log.error(("MicrogamingToFpdb.determineGameType: '%s'") % tmp)
    ...
    def readHandInfo(self, hand):
        ...
        if m is None:
            ...
            log.error(("MicrogamingToFpdb.readHandInfo: '%s'") % tmp)
            raise FpdbParseError
    ...
    def readAction(self, hand, street):
        ...
        for action in m:
            ...
            if action.group('ATYPE') == 'Fold':
                ...
                log.debug(("DEBUG:") + ("Unimplemented %s: '%s' '%s'") % ("readAction", pname, action.group('ATYPE')))
