...
if not options.errorsToConsole:
    print((("Note: error output is being diverted to %s.") % "tourneyerror.txt"), end=' ')
...
#    if there is a db error, complain, skip hand, and proceed
            try:
                ...
                if new_hand_id: # new_hand_id is none if we had an error prior to the store
                    sys.stderr.write(("Database error %s in hand %d. Skipping.") % (err, int(new_hand_id)) + "\n")
                continue
            ...
            if type == "tour":   # hand is from a tournament
                ...
                if mat_obj:
                    ...
                    sys.stderr.write(("Could not find tournament %d in hand %d. Skipping.") % (int(tour_number), int(new_hand_id)) + "\n")
                    continue
...
#    If no client window is found on the screen, complain and continue
                    ...
                    sys.stderr.write(("Table name %s not found, skipping.")% table_name)
                else:
