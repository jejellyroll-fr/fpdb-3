...
class Boss(HandHistoryConverter):
    ...
    re_PostDSB          = re.compile(r'^<ACTION TYPE="HAND_BLINDS" PLAYER="%s" KIND="HAND_DSB" VALUE="(?P<DSB>[.0-9]+)"></ACTION>' %  player_re, re.MULTILINE)
    ...
    def determineGameType(self, handText):
        ...
        if not m:
            tmp = handText[0:200]
            log.error(("BossToFpdb.determineGameType: '%s'") % tmp)
        ...
        if info['limitType'] == 'fl' and info['bb'] is not None:
            if info['type'] == 'ring':
                try:
                    ...
                    tmp = handText[0:200]
                    log.error(("BossToFpdb.determineGameType: Lim_Blinds has no lookup for '%s' - '%s'") % (mg['BB'], tmp))
                    raise FpdbParseError
            else:
    ...
    def readHandInfo(self, hand):
        ...
        if m is None:
            ...
            log.error(("BossToFpdb.readHandInfo: '%s'") % tmp)
            raise FpdbParseError
    ...
    def calculateAntes(self, street, hand):
        if street in ('PREFLOP', 'DEAL'):
            ...
            try:
                ...
                log.error(("BossToFpdb.calculateAntes(): '%s': Major failure while calculating pot: '%s'") % (self.handid, e))
                raise FpdbParseError
            if street=='DEAL':
