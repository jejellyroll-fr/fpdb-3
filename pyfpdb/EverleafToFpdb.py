...
class Everleaf(HandHistoryConverter):
    ...
    substitutions = {
                     'LEGAL_ISO' : "USD|EUR|GBP|CAD|FPP",       # legal ISO currency codes
                            'LS' : u"\$|\u20AC|€|\x80|\u02c6|",  # legal currency symbols - Euro(cp1252, utf-8) #TODO change \x80 to \x20\x80, update all regexes accordingly
                        'PLAYERS': r'(?P<PNAME>.+?)',
...
or None if we fail to get the info """
        ...
        if not m:
            tmp = handText[0:200]
            log.error(("EverleafToFpdb.determineGameType: '%s'") % tmp)
        ...
        if info['limitType'] == 'fl' and info['bb'] is not None:
            if info['type'] == 'ring':
                try:
                    ...
                    tmp = handText[0:200]
                    log.error(("EverleafToFpdb.determineGameType: Lim_Blinds has no lookup for '%s' - '%s'") % (mg['BB'], tmp))
                    raise FpdbParseError
            else:
    ...
    def readHandInfo(self, hand):
        ...
        if(m == None):
            ...
            log.error(("EverleafToFpdb.readHandInfo: '%s'") % tmp)
            raise FpdbParseError
...
#            hand.addHoleCards(cards, m.group('PNAME'))
    ...
    def readStudPlayerCards(self, hand, street):
        log.warning(("%s cannot read all stud/razz hands yet.") % hand.sitename)
    ...
    def readAction(self, hand, street):
        ...
        for action in m:
            ...
            if action.group('ATYPE') == ' folds':
                ...
                log.debug(("Unimplemented %s: '%s' '%s'") % ("readAction", action.group('PNAME'), action.group('ATYPE')))
