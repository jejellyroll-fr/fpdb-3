...
class DerivedStats(object):
                    ...
                    if (pname == hand.hero and streetSeen) or (hp["showed"] and streetSeen) or hp["sawShowdown"]:
                        ...
                        for n in range(len(board["board"])):
                            ...
                            if notnull and (postflop or maxcards):
                                ...
                                self.handsstove.append(
                                    [hand.dbid_hands, hand.dbid_pids[pname], streetId, boardId, "n", 1, 0, None, 0]
                                )
            else:
            ...
            for street in allInStreets:
                ...
                for n in range(len(board["board"])):
                    ...
                    valid = [
                        p
                        for p in players
                        if self.handsplayers[p]["sawShowdown"] and "0x" not in holecards[p]["cards"][n]
                    ]
                    if potId == 1:
                        allplayers = valid
                        ...
                        for i in range(len(equities)):
                            ...
                            if street == startstreet:
                                rake = (
                                    Decimal(0)
                                    if hand.cashedOut
                                    else (hand.rake * (old_div(Decimal(pot), Decimal(hand.totalpot))))
                                )
    ...
    def assembleHandsPots(self, hand):
        ...
        # process pots
        if (
            (hand.sitename != "KingsClub" or hand.adjustCollected)
            and evalgame
