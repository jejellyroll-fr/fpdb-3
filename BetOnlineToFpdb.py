        ...
        if info['limitType'] == 'fl' and info['bb'] is not None:
            if info['type'] == 'ring':
                try:
                    ...
                    tmp = handText[0:200]
                    log.error(("BetOnlineToFpdb.determineGameType: Lim_Blinds has no lookup for '%s' - '%s'") % (mg['BB'], tmp))
                    raise FpdbParseError
            else:
                ...
                if hand.tourNo!=None:
                    ...
                    if not info[key] or info[key] == 'Freeroll':
                        ...
                        if info[key].find("$")!=-1:
                            ...
                            raise FpdbParseError(("BetOnlineToFpdb.readHandInfo: Failed to detect currency.") + " " + ("Hand ID") + ": %s: '%s'" % (hand.handid, info[key]))
