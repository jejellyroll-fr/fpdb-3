...
class BetOnline(HandHistoryConverter):
    ...
    re_PlayerInfo = re.compile(
        r"""
          ^Seat\s(?P<SEAT>[0-9]+):\s
          (?P<PNAME>.*)\s
          \((%(LS)s)?(?P<CASH>[%(NUM)s]+)\sin\s[cC]hips\)"""
        % substitutions,
        re.MULTILINE | re.VERBOSE,
    )
    ...
    re_DateTime1 = re.compile(
        r"""(?P<Y>[0-9]{4})\/(?P<M>[0-9]{2})\/(?P<D>[0-9]{2})[\- ]+(?P<H>[0-9]+):(?P<MIN>[0-9]+)(:(?P<S>[0-9]+))?\s(?P<TZ>.*$)""",
        re.MULTILINE,
    )
    ...
    re_JoinsTable = re.compile(r"^.+ joins the table at seat #\d+", re.MULTILINE)
    ...
    re_CollectPot = re.compile(
        r"Seat (?P<SEAT>[0-9]+): %(PLYR)s (\(.+?\)  )?(collected|showed \[.*\] and won) \((%(LS)s)?(?P<POT>[%(NUM)s]+)\)"
        % substitutions,
        re.MULTILINE,
    )
